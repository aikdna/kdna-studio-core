...
function detectContradictions(cards) {
  ...
  for (const ax of axioms) {
    if (
      !ax.fields?.does_not_apply_when ||
      ax.fields.does_not_apply_when.length === 0
    ) {
      issues.push({
    ...
    if (
      /\b(always|never|every|all|none|must)\b/.test(oneLiner) &&
      (!ax.fields.does_not_apply_when ||
        ax.fields.does_not_apply_when.length === 0)
    ) {
      issues.push({
  ...
  const misunderstandings = cards.filter(
    (c) => c.type === "misunderstanding" && c.locked,
  );
  ...
  for (const bd of boundaries) {
    ...
    if (
      !bd.fields?.acceptable_exceptions ||
      bd.fields.acceptable_exceptions.length === 0
    ) {
      issues.push({
        type: "no_exceptions",
        card_id: bd.id,
