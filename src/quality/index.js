...
function computeReadiness(project) {
  ...
  const lockedMisunderstandings = locked.filter(
  ...
  if (sourceMode === "source_folder") {
    warnings.push(
      "source_folder: imported cards have no inherited review provenance",
    );
    warnings.push(
      "source_folder: review the schema audit before making provenance claims",
    );
  }
  if (sourceMode === "kdna_asset") {
    const hasRelevantLineage =
      project.lineage &&
      (project.lineage.parent_name || project.lineage.parent_asset_uid);
    if (!hasRelevantLineage) {
      blocking.push(
        "kdna_asset: lineage missing — must record parent KDNA identity",
      );
    }
    warnings.push(
      "kdna_asset: imported cards do not inherit parent review provenance",
    );
  }
  ...
  const declaredLanguages = (project.languages || []).filter(
    (l) => l !== project.default_language,
  );
  const hasMultiLangIntent =
    declaredLanguages.length > 0 ||
    (project.i18n_level && project.i18n_level !== "L0");
  if (hasMultiLangIntent && i18nCoverage.level === "L0") {
    warnings.push(
      "I18N: project declares multi-language intent but has no locale files (L0). Add at least L1 (KDNA_CARD.json + README in locales/).",
    );
  } else if (hasMultiLangIntent && i18nCoverage.level === "L1") {
    warnings.push(
      "I18N: L1 achieved (card + readme). Recommended: L2 overlay for publishable grade.",
    );
  }
  for (const issue of govResult.issues) {
    (issue.severity === "blocking" ? blocking : warnings).push(
      `Governance: ${issue.message}`,
    );
  }
  ...
  for (const { card_id, issues } of cardResults) {
    for (const issue of issues) {
      if (issue.severity === "blocking")
        blocking.push(`${card_id}: ${issue.message}`);
  ...
  if (locked.length === 0)
    warnings.push(
      "No Human Lock provenance is recorded; compilation remains allowed",
    );

  // ── Axiom Checks ──────────────────────────────────────────────
  for (const ax of lockedAxioms) {
    if (!ax.fields?.one_sentence || ax.fields.one_sentence.length < 10)
      blocking.push(`${ax.id}: one_sentence too short`);
    if (!ax.fields?.applies_when?.length)
  ...
  for (const ms of lockedMisunderstandings) {
    if (!ms.fields?.key_distinction || ms.fields.key_distinction.length < 20)
      blocking.push(`${ms.id}: key_distinction too short`);
  }
  ...
  // ── Determine Grade ────────────────────────────────────────────
  const axiomsComplete =
    lockedAxioms.length >= 1 &&
    lockedAxioms.every(
      (ax) =>
  ...
  const feynmanRatio =
    lockedAxioms.length > 0
      ? lockedAxioms.filter((ax) => ax.feynman_restatement).length /
        lockedAxioms.length
      : 0;
  const allFeynman =
    lockedAxioms.every((ax) => ax.feynman_restatement) &&
    lockedMisunderstandings.every((ms) => !ms.locked || ms.feynman_restatement);
  ...
  const misunderstandingFeynmanQuality =
    lockedMisunderstandings.length === 0 ||
  ...
  if (allFeynman && !feynmanQuality) {
    warnings.push(
      "Feynman: axiom restatements should score ≥4/5 for publishable grade",
    );
  }

  // Compare test results requirements (0.6.4)
  const withKdnaBetter = ratedTests.filter(
    (t) => t.result === "with_kdna_better",
  ).length;
  const withoutKdnaBetter = ratedTests.filter(
    (t) => t.result === "without_kdna_better",
  ).length;
  if (ratedTests.length > 0 && withoutKdnaBetter > 0) {
    warnings.push(
      `${withoutKdnaBetter} test(s) were preferred by the named evaluator without KDNA; this is scoped assessment evidence, not intrinsic asset quality`,
    );
  }
  if (ratedTests.length > 0 && withKdnaBetter < 3 && ratedTests.length >= 5) {
    warnings.push(
      `Only ${withKdnaBetter} tests favor KDNA — recommend ≥3 for confidence`,
    );
  }

  let grade = "draft_grade";
  if (locked.length >= 3 && axiomsComplete && feynmanRatio >= 0.5)
    grade = "human_controlled";
  if (
    grade === "human_controlled" &&
    ratedTests.length >= 5 &&
    lockedSelfChecks.length >= 3
  )
    grade = "tested_grade";
  if (
    grade === "tested_grade" &&
    ...
    allFeynman &&
    feynmanQuality &&
    misunderstandingFeynmanQuality
  ) {
  ...
  if (grade === "publishable_grade" && govResult && !govResult.valid) {
    grade = "tested_grade";
    warnings.push(
      "Governance checks not passed — publishable downgraded to tested",
    );
  }
...
function buildResult(grade, blocking, warnings, project, detail = {}) {
  ...
  return {
    ...
    stats: {
      ...
      locked_axioms: (project.cards || []).filter(
        (c) => c.type === "axiom" && c.locked,
      ).length,
      locked_self_checks: (project.cards || []).filter(
        (c) => c.type === "self_check" && c.locked,
      ).length,
      total_tests: (project.tests || []).length,
      rated_tests: ratedTests,
      feynman_ratio:
        detail.feynmanRatio !== undefined
          ? Math.round(detail.feynmanRatio * 100) + "%"
          : "N/A",
      i18n_level: detail.i18n?.level || "L0",
    },
    next_step:
      grade === "draft_grade"
        ? "Lock at least 3 axioms with boundaries and 50% Feynman."
        : grade === "human_controlled"
          ? "Add 5+ rated evals and 3+ self-checks."
          : grade === "tested_grade"
            ? "Add 10+ evals, complete Feynman on all axioms/misunderstandings, resolve all blocking issues."
...
function getBlockingIssues(project) {
  return computeReadiness(project).blocking;
}
