...
const CATEGORY_MINIMUMS = {
  positive_target: {
    min: 8,
    label: "Positive target cases",
    description: "Tasks clearly in the asset domain",
  },
  non_applicable: {
    min: 4,
    label: "Non-applicable cases",
    description: "Tasks clearly outside scope — asset should skip",
  },
  adjacent_ambiguous: {
    min: 4,
    label: "Adjacent/ambiguous cases",
    description: "Tasks near the boundary — tests boundary behavior",
  },
  high_risk_failure: {
    min: 2,
    label: "High-risk failure cases",
    description: "Tasks where wrong judgment would cause harm",
  },
  regression: {
    min: 2,
    label: "Regression cases",
    description: "Tasks from previously observed real failures",
  },
  holdout: {
    min: 1,
    label: "Holdout split",
    description: "Independent cases not used for authoring or repair",
  },
};
...
function checkMinimumCaseSet(project) {
  ...
  for (const [cat, config] of Object.entries(CATEGORY_MINIMUMS)) {
    ...
    summary[cat] = {
      current: count,
      required: config.min,
      met: count >= config.min,
    };
    if (count < config.min) {
      missing.push(cat);
      errors.push(
        `${config.label}: ${count}/${config.min} (${config.description})`,
      );
    }
  }
  ...
  return {
    ...
    total_required: Object.values(CATEGORY_MINIMUMS).reduce(
      (s, c) => s + c.min,
      0,
    ),
  };
