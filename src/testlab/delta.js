...
function parseCompareOutput(diffText) {
  ...
  for (const line of lines) {
    ...
    if (dotIndex !== 1 || colonIndex <= dotIndex + 1 || !isDigit(line[0]))
      continue;
...
function buildScoring(axes, domainScore, selfChecksPassed) {
  return {
    ...
    D5_self_check_pass_rate:
      selfChecksPassed !== null ? `${selfChecksPassed}%` : "N/A",
    D6_boundary_respect:
      axes.boundary_awareness || axes.boundary ? "Pass" : "N/A",
...
function formatDeltaMarkdown(delta) {
  ...
  if (!delta.changed_dimensions.length)
...
function parseCompareReportJson(report) {
  ...
  if (report.diff.axes) {
    for (const [axis, value] of Object.entries(report.diff.axes)) {
      if (value && String(value).toUpperCase() !== "SAME")
...
function createJudgmentDeltaFromReport(domain, input, report, options = {}) {
  ...
  return {
    meta: {
      domain,
      input: (input || "").slice(0, 200),
      model: report.meta?.model || options.model || "unknown",
      timestamp: new Date().toISOString(),
    },
    classification: {
      without_kdna: axes.classification || "generic",
...
module.exports = {
  parseCompareOutput,
  parseCompareReportJson,
  scoreDelta,
  createJudgmentDelta,
  createJudgmentDeltaFromReport,
  compareDeltas,
  formatDeltaMarkdown,
};
