  ...
  try {
    if (typeof Intl !== "undefined" && Intl.Segmenter) {
      ...
      return [...segmenter.segment(text)]
        .filter((s) => s.isWordLike && s.segment.length > 3)
        .map((s) => s.segment);
    }
  } catch {
    /* fallback */
...
function createFeynmanRestatement(card, text) {
  if (!text || typeof text !== "string")
    throw new Error("Feynman restatement text is required");
  if (text.length < 20)
    throw new Error("Feynman restatement too short (minimum 20 chars)");
...
function evaluateRestatementQuality(card, text) {
  ...
  const overlapRatio =
    textWords.length > 0 ? overlapCount / textWords.length : 1;
  ...
  const hasStory =
    storyPatterns.filter((w) => textLower.includes(w)).length >= 2;
...
function attachRestatementToLock(card, restatement) {
  if (!card.human_lock)
    throw new Error("Card must be locked before attaching Feynman restatement");
...
function validateRestatementCard(card) {
  ...
  if (!card.feynman_restatement) {
    issues.push({
      type: "missing_feynman",
      severity: "warning",
      message: `${card.id}: missing Feynman restatement (lock is stronger with it)`,
    });
    return issues;
  }
  const fr = card.feynman_restatement;
  if (!fr.score?.not_just_repeat)
    issues.push({
      type: "repeat",
      severity: "warning",
      message: `${card.id}: Feynman may just repeat the original text`,
    });
  if (!fr.score?.not_too_abstract)
    issues.push({
      type: "abstract",
      severity: "warning",
      message: `${card.id}: Feynman may be too abstract`,
    });
  if (!fr.score?.clarifies_boundary)
    issues.push({
      type: "no_boundary",
      severity: "warning",
      message: `${card.id}: Feynman does not explain when this does NOT apply`,
    });
  if (!fr.score?.ordinary_person_understands)
    issues.push({
      type: "complex",
      severity: "warning",
      message: `${card.id}: Feynman may be too complex for a non-expert`,
    });
  return issues;
}
