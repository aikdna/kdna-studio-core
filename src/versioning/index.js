...
function diffProjects(oldProject, newProject) {
  ...
  for (const [id, nc] of newById) {
    if (!oldById.has(id)) {
      ...
      if (Object.keys(fieldChanges).length > 0) {
        ...
        changed.push({
          ...cardSummary(nc),
          status_change: { from: oc.status, to: nc.status },
        });
      }
    }
  }
  ...
  return {
    ...
    removed,
    changed,
    unchanged: oldCards.length - removed.length - changed.length,
    summary: {
...
function cardSummary(card, changes) {
  return {
    id: card.id,
    type: card.type,
    one_sentence: card.fields?.one_sentence || card.fields?.question || "",
...
function diffFields(oldFields, newFields) {
  ...
  for (const key of new Set([
    ...
    const ov = stableStringify(oldFields[key] || null),
      nv = stableStringify(newFields[key] || null);
    if (ov !== nv)
      changes[key] = {
...
function recommendVersionBump(diff) {
  ...
  const removedMisunderstandings = removed.filter(
    (c) => c.type === "misunderstanding",
  );

  // MAJOR checks
  if (removedAxioms.length > 0 || removedMisunderstandings.length > 0)
    return "major";
  for (const c of changed) {
    ...
    if ("applies_when" in c.changes) {
      const bef = c.changes.applies_when.before || [],
        aft = c.changes.applies_when.after || [];
      if (aft.length > bef.length) return "major";
    }
    // Removed boundary → major
    if ("does_not_apply_when" in c.changes) {
      const bef = c.changes.does_not_apply_when.before || [],
        aft = c.changes.does_not_apply_when.after || [];
      if (aft.length < bef.length) return "major";
    }
  }
  ...
  const addedMisunderstandings = added.filter(
    (c) => c.type === "misunderstanding",
  );
  const addedSelfChecks = added.filter((c) => c.type === "self_check");
  if (
    addedAxioms.length > 0 ||
    addedMisunderstandings.length > 0 ||
    addedSelfChecks.length > 0
  )
    return "minor";
  for (const c of changed) {
    if (!c.changes) continue;
    ...
    if (c.type === "misunderstanding" && "key_distinction" in c.changes)
      return "minor";
  }
...
function generateChangelog(diff, oldVersion, newVersion, options = {}) {
  ...
  for (const [label, items] of [
    ...
    for (const c of items) {
      ...
      if (c.status_change)
        lines.push(
          `  - Status: ${c.status_change.from} → ${c.status_change.to}`,
        );
      if (c.changes)
        for (const [f, v] of Object.entries(c.changes)) {
          lines.push(
            `  - ${f}: "${String(v.before || "").slice(0, 60)}" → "${String(v.after || "").slice(0, 60)}"`,
          );
        }
    }
    lines.push("");
...
function markBreakingChange(diff) {
  ...
  const scopeWidening = diff.changed.filter(
    (c) =>
      c.changes &&
      "applies_when" in c.changes &&
      (c.changes.applies_when.after || []).length >
        (c.changes.applies_when.before || []).length,
  );
  const coreMeaningChanges = diff.changed.filter(
    (c) =>
      c.changes &&
      ("one_sentence" in c.changes || "full_statement" in c.changes),
  );
  ...
  return {
    ...
    reason:
      removedAxioms.length > 0
        ? `${removedAxioms.length} axiom(s) removed — breaking change`
        : coreMeaningChanges.length > 0
          ? `${coreMeaningChanges.length} core meaning change(s) — breaking change`
          : scopeWidening.length > 0
            ? `${scopeWidening.length} scope widening(s) — may affect existing behavior`
            : null,
