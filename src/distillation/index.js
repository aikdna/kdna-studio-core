...
const OWNER_SCOPES = Object.freeze({
  personal: {
    ...
    description:
      "One person's individual standards. Extracts personal preferences, boundaries, taste.",
  },
  team: {
    id: "team",
    displayName: "Team",
    description:
      "Shared team conventions. Extracts team-wide standards, agreed practices.",
  },
  organization: {
    id: "organization",
    displayName: "Organization",
    description:
      "Company/organization policies. Extracts organizational values, compliance boundaries.",
  },
  field: {
    id: "field",
    displayName: "Industry / Field",
    description:
      "Industry/profession-wide. Extracts domain expertise beyond any single practitioner.",
  },
});
...
const GRANULARITY_LEVELS = Object.freeze({
  core_principles: {
    ...
    description:
      "High-level axioms and boundaries. Foundational beliefs, what the person consistently prioritizes and rejects.",
  },
  concrete_patterns: {
    id: "concrete_patterns",
    displayName: "Concrete Patterns",
    description:
      "Recurring decision patterns. Specific rules and detectable habits.",
  },
  specific_scenarios: {
    id: "specific_scenarios",
    displayName: "Specific Scenarios",
    description:
      "Scenario-level triggers. Context-specific judgment triggers and responses.",
  },
});
...
function createDistillationTarget({
  ...
  if (!DOMAIN_CATEGORIES[domainCategory]) {
    throw new Error(
      `Invalid domainCategory: ${domainCategory}. Must be one of: ${Object.keys(DOMAIN_CATEGORIES).join(", ")}`,
    );
  }
  ...
  return {
    ...
    load_condition:
      loadCondition ||
      `Load when the task involves ${DOMAIN_CATEGORIES[domainCategory].displayName.toLowerCase()} judgment.`,
...
function validateDistillationTarget(target) {
  ...
  if (!DOMAIN_CATEGORIES[target.domain_category])
    errors.push(`invalid domain_category: ${target.domain_category}`);
  if (!OWNER_SCOPES[target.owner_scope])
    errors.push(`invalid owner_scope: ${target.owner_scope}`);
  if (!GRANULARITY_LEVELS[target.granularity])
    errors.push(`invalid granularity: ${target.granularity}`);
  if (!target.task_scope || target.task_scope.trim().length === 0)
    errors.push("task_scope is required");
...
function applyScopeGate(candidate, target) {
  const text =
    `${candidate.one_sentence || ""} ${candidate.full_statement || ""}`.toLowerCase();
  ...
  const excludeMatch = (target.exclude_areas || []).some((w) =>
  ...
  if (!scopeFit) {
    for (const [catId, cat] of Object.entries(DOMAIN_CATEGORIES)) {
      if (catId !== target.domain_category) {
        if (
          text.includes(catId) ||
          text.includes(cat.displayName.toLowerCase())
        ) {
  ...
  for (const [domain, keywords] of Object.entries(SENSITIVE_KEYWORDS)) {
    for (const keyword of keywords) {
      if (lower.includes(keyword.toLowerCase())) {
        return {
          flagged: true,
          reason: `May involve sensitive domain: ${domain}`,
        };
      }
    }
  }
