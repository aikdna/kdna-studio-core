...
const PROJECT_SCHEMA_PATH = path.resolve(
  __dirname,
  "..",
  "schemas",
...
function parseProjectSchemaAuthority(bytes) {
  const schemaBytes = Buffer.isBuffer(bytes)
    ? Buffer.from(bytes)
    : Buffer.from(String(bytes));
  const schema = JSON.parse(schemaBytes.toString("utf8"));
  const declaredCardTypes =
    schema?.properties?.cards?.items?.properties?.type?.enum;

  if (!Array.isArray(declaredCardTypes) || declaredCardTypes.length !== 16) {
    throw new Error(
      "studio.project.schema.json must declare exactly 16 card types",
    );
  }
  ...
  if (
    declaredCardTypes.some(
      (type) => typeof type !== "string" || type.length === 0,
    )
  ) {
    throw new Error(
      "studio.project.schema.json card types must be non-empty strings",
    );
  }
...
const authority = parseProjectSchemaAuthority(
  fs.readFileSync(PROJECT_SCHEMA_PATH),
);
