  ...
  for (const card of cards) {
    ...
    for (const field of TEXT_FIELDS) {
      if (
        ...
        fields[field].length > 3
      ) {
        overlay.translations[`${card.id}.${field}`] =
          `[TODO: ${locale}] ${fields[field]}`;
      }
    }
      ...
      if (Array.isArray(fields[field])) {
        fields[field].forEach((val, idx) => {
          if (val && typeof val === "string") {
            overlay.translations[`${card.id}.${field}.${idx}`] =
              `[TODO: ${locale}] ${val}`;
          }
        });
      }
...
function validateLocaleOverlay(project, overlay) {
  ...
  if (!overlay.locale)
    issues.push({
      type: "missing_locale",
      severity: "blocking",
      message: "Overlay must declare locale",
    });
  if (!overlay.translations || Object.keys(overlay.translations).length === 0) {
    issues.push({
      type: "empty",
      severity: "warning",
      message: "Overlay has no translations",
    });
  ...
  for (const key of Object.keys(overlay.translations)) {
    ...
    if (!cardIds.has(cardId)) {
      issues.push({
        type: "unknown_id",
        severity: "blocking",
        message: `Overlay references unknown card: ${cardId}`,
      });
    }
  }

  // Check for TODO placeholders
  const todoCount = Object.values(overlay.translations).filter((v) =>
    v.includes("[TODO:"),
  ).length;
  if (todoCount > 0) {
    issues.push({
      type: "incomplete",
      severity: "warning",
      message: `${todoCount} translations still have TODO placeholders`,
    });
  }
    ...
    if (typeof obj[key] === "string") {
      ...
      if (obj[key].every((v) => typeof v === "string")) {
        obj[key] = obj[key].map((v, i) => {
          ...
          return translations[arrayKey] &&
            !translations[arrayKey].includes("[TODO:")
            ? translations[arrayKey]
            : v;
        });
      } else {
...
function computeI18nCoverage(project) {
  ...
  if (locked.length === 0)
    return { level: "L0", coverage: 0, translatable_fields: 0 };

  const totalFields = locked.reduce((sum, c) => {
    const fields = c.fields || {};
    let count = 0;
    for (const f of TEXT_FIELDS) {
      if (fields[f] && typeof fields[f] === "string") count++;
    }
    for (const f of ARRAY_TEXT_FIELDS) {
      if (Array.isArray(fields[f]))
        count += fields[f].filter((v) => typeof v === "string").length;
    }
    return sum + count;
  }, 0);
  ...
  const coverage =
    totalFields > 0 ? Math.round((translatedCount / totalFields) * 100) : 0;
  ...
  return {
    ...
    translatable_fields: totalFields,
