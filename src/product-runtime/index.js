...
function createProductRuntime(options = {}) {
  ...
  if (!/^[a-z][a-z0-9-]{0,63}$/.test(name)) {
    throw new Error(
      `Invalid runtime name "${name}": must match ^[a-z][a-z0-9-]{0,63}$`,
    );
  }
  ...
  const manifest = {
    ...
    schedule: {
      ...
      ...(options.scheduleType === "cron" && options.cron
        ? { cron: options.cron }
        : {}),
      ...(options.scheduleType === "interval"
        ? { interval_seconds: options.intervalSeconds || 3600 }
        : {}),
      ...(options.scheduleType === "event_driven" && options.eventTrigger
    ...
    selection: {
      ...
      ...(options.contextSignals
        ? { context_signals: options.contextSignals }
        : {}),
      ...(options.userStatePath
        ? { user_state_path: options.userStatePath }
        : {}),
    },
    generation: {
    ...
    delivery: {
      ...
      ...(options.deliveryTemplate
        ? { template: options.deliveryTemplate }
        : {}),
      ...(options.deliveryMetadata
        ? { metadata: options.deliveryMetadata }
        : {}),
    },
  };
...
function validateProductRuntime(manifest) {
  ...
  if (manifest.format !== "kdna-product-runtime") {
    issues.push(
      'format: must be "kdna-product-runtime", got ' +
        JSON.stringify(manifest.format),
    );
  }
  if (manifest.format_version !== "0.1") {
    issues.push(
      'format_version: must be "0.1", got ' +
        JSON.stringify(manifest.format_version),
    );
  }
  if (!manifest.name || !/^[a-z][a-z0-9-]{0,63}$/.test(manifest.name)) {
    issues.push(
      `name: invalid — must match ^[a-z][a-z0-9-]{0,63}$, got ${JSON.stringify(manifest.name)}`,
    );
  }
  if (!manifest.version) {
  ...
  if (!sched || typeof sched !== "object") {
    ...
    if (!SCHEDULE_TYPES.includes(sched.type)) {
      issues.push(
        `schedule.type: must be one of [${SCHEDULE_TYPES.join(", ")}], got ${JSON.stringify(sched.type)}`,
      );
    }
    if (!SKIP_POLICIES.includes(sched.skip_policy)) {
      issues.push(
        `schedule.skip_policy: must be one of [${SKIP_POLICIES.join(", ")}]`,
      );
    }
    if (
      typeof sched.max_backlog !== "number" ||
      sched.max_backlog < 0 ||
      !Number.isInteger(sched.max_backlog)
    ) {
    ...
    if (sched.type === "interval") {
      if (
        typeof sched.interval_seconds !== "number" ||
        sched.interval_seconds < 60 ||
        !Number.isInteger(sched.interval_seconds)
      ) {
        issues.push(
          'schedule.interval_seconds: required integer >= 60 when type is "interval"',
        );
      }
    }
    if (sched.type === "event_driven" && !sched.event_trigger) {
      issues.push(
        'schedule.event_trigger: required when type is "event_driven"',
      );
    }
  }
  ...
  if (!sel || typeof sel !== "object") {
    ...
    if (!SELECTION_TYPES.includes(sel.type)) {
      issues.push(
        `selection.type: must be one of [${SELECTION_TYPES.join(", ")}]`,
      );
    }
    if (!Array.isArray(sel.domain_pool) || sel.domain_pool.length < 1) {
      issues.push(
        "selection.domain_pool: required array with at least 1 domain",
      );
    } else {
      sel.domain_pool.forEach((d, i) => {
        ...
        if (d.role && !DOMAIN_ROLES.includes(d.role)) {
          issues.push(
            `selection.domain_pool[${i}].role: must be one of [${DOMAIN_ROLES.join(", ")}]`,
          );
        }
      });
    }
    ...
    if (sel.rotation && !ROTATION_MODES.includes(sel.rotation)) {
      issues.push(
        `selection.rotation: must be one of [${ROTATION_MODES.join(", ")}]`,
      );
    }
  }
  ...
  if (!gen || typeof gen !== "object") {
    ...
    if (
      gen.kdna_load_profile &&
      !LOAD_PROFILES.includes(gen.kdna_load_profile)
    ) {
      issues.push(
        `generation.kdna_load_profile: must be one of [${LOAD_PROFILES.join(", ")}]`,
      );
    }
    if (
      typeof gen.max_generation_attempts === "number" &&
      (!Number.isInteger(gen.max_generation_attempts) ||
        gen.max_generation_attempts < 1)
    ) {
      issues.push(
        "generation.max_generation_attempts: must be a positive integer",
      );
    }
    if (Array.isArray(gen.quality_gates)) {
      gen.quality_gates.forEach((g, i) => {
        if (!GATE_TYPES.includes(g.gate_type)) {
          issues.push(
            `generation.quality_gates[${i}].gate_type: must be one of [${GATE_TYPES.join(", ")}]`,
          );
        }
        if (typeof g.blocking !== "boolean") {
          issues.push(
            `generation.quality_gates[${i}].blocking: must be boolean`,
          );
        }
      });
    }
  ...
  if (!del || typeof del !== "object") {
    ...
    if (!DELIVERY_TYPES.includes(del.type)) {
      issues.push(
        `delivery.type: must be one of [${DELIVERY_TYPES.join(", ")}]`,
      );
    }
  }
  ...
  if (manifest.observation) {
    ...
    if (Array.isArray(obs.sources)) {
      obs.sources.forEach((s, i) => {
        if (!OBSERVATION_SOURCES.includes(s)) {
          issues.push(
            `observation.sources[${i}]: must be one of [${OBSERVATION_SOURCES.join(", ")}]`,
          );
        }
      });
    }
    if (Array.isArray(obs.signal_mapping)) {
      obs.signal_mapping.forEach((sm, i) => {
        if (!sm.source)
          issues.push(`observation.signal_mapping[${i}].source: required`);
        if (!sm.extract)
          issues.push(`observation.signal_mapping[${i}].extract: required`);
        if (!sm.maps_to)
          issues.push(`observation.signal_mapping[${i}].maps_to: required`);
      });
    }
  }
  ...
  if (manifest.adaptation) {
    ...
    if (
      typeof adp.max_adaptation_depth === "number" &&
      (!Number.isInteger(adp.max_adaptation_depth) ||
        adp.max_adaptation_depth < 1)
    ) {
      issues.push(
        "adaptation.max_adaptation_depth: must be a positive integer",
      );
    }
    if (Array.isArray(adp.adaptation_rules)) {
      adp.adaptation_rules.forEach((r, i) => {
        if (!r.signal_type)
          issues.push(
            `adaptation.adaptation_rules[${i}].signal_type: required`,
          );
        if (!r.condition)
          issues.push(`adaptation.adaptation_rules[${i}].condition: required`);
        if (!ADAPTATION_ACTIONS.includes(r.action)) {
          issues.push(
            `adaptation.adaptation_rules[${i}].action: must be one of [${ADAPTATION_ACTIONS.join(", ")}]`,
          );
        }
      });
    }
...
function compactProductRuntime(manifest) {
  const copy = {
    ...
    selection: {
      ...manifest.selection,
      domain_pool: [...(manifest.selection.domain_pool || [])],
    },
  ...
  if (
    !copy.selection.context_signals ||
    copy.selection.context_signals.length === 0
  ) {
  ...
  if (
    !copy.generation.quality_gates ||
    copy.generation.quality_gates.length === 0
  ) {
    delete copy.generation.quality_gates;
  }
  if (!copy.delivery.template) delete copy.delivery.template;
  if (
    !copy.delivery.metadata ||
    Object.keys(copy.delivery.metadata).length === 0
  ) {
    delete copy.delivery.metadata;
  }
  if (
    !copy.observation ||
    (!copy.observation.sources && !copy.observation.signal_mapping)
  ) {
    delete copy.observation;
  }
  if (
    !copy.adaptation ||
    (copy.adaptation.enabled !== true && !copy.adaptation.adaptation_rules)
  ) {
