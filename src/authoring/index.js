...
function ensureAuthoringStages(project) {
  ...
  project.stages.source = project.stages.source || {
    status: "pending",
    judgment_count: 0,
  };
  project.stages.review = project.stages.review || {
    status: "pending",
    reviewed_count: 0,
  };
  project.stages.confirm = project.stages.confirm || {
    status: "pending",
    confirmed_count: 0,
  };
...
function addSourceJudgment(project, input = {}) {
  ...
  if (!String(statement).trim().includes(" ")) {
    throw new Error(
      "statement must be a complete testable explanation, not a single label",
    );
  }
  const rationale = nonEmpty(input.rationale, "rationale");
  if (!String(rationale).trim().includes(" ")) {
    throw new Error(
      "rationale must be a complete explanation, not a single label",
    );
  }
  ...
  const source = {
    ...
    ...(input.sourceReference
      ? { reference: nonEmpty(input.sourceReference, "sourceReference") }
      : {}),
  };
  const represents = input.represents
    ? {
        ...
        ...(input.represents.name
          ? { name: nonEmpty(input.represents.name, "represents.name") }
          : {}),
        status: "unconfirmed",
      }
    : null;
  ...
  project.stages.source.judgment_count = project.cards.filter(
...
function reviewJudgment(project, cardId, review = {}) {
  ...
  if (card.status !== "draft")
    throw new Error(`judgment ${cardId} is not awaiting review`);
  const reviewedBy = nonEmpty(review.by, "review.by");
  const reviewStatement = nonEmpty(review.statement, "review.statement");
  const reviewed = transitionCard(card, "revised", {
    by: reviewedBy,
    reason: reviewStatement,
  });
  reviewed.review = {
    by: reviewedBy,
    at: new Date().toISOString(),
  ...
  project.stages.review.reviewed_count = project.cards.filter(
    (item) => item.review,
  ).length;
  return reviewed;
}
...
function confirmJudgment(project, cardId, confirmation = {}) {
  ...
  const confirmationStatement = nonEmpty(
  ...
  project.stages.confirm.confirmed_count = project.cards.filter(
...
function assertConfirmedForExport(project) {
  ...
  if (active.length === 0)
    throw new Error("cannot export a Studio project with no judgments");
  const problems = [];
  for (const card of active) {
    if (!card.fields?.source_provenance)
      problems.push(`${card.id}: source provenance is missing`);
    if (!card.review) problems.push(`${card.id}: review is missing`);
    if (
      !card.confirmation ||
      !["locked", "tested", "published"].includes(card.status)
    ) {
    ...
    if (representation && representation.status !== "confirmed") {
      problems.push(
        `${card.id}: represented subject ${representation.id} is not confirmed`,
      );
    }
  }
  if (problems.length > 0) {
    throw new Error(
      `Studio export requires reviewed and confirmed judgments:\n  - ${problems.join("\n  - ")}`,
    );
  }
}
