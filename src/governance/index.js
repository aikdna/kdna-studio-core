...
function computeRiskLevel(project) {
  ...
  const declared =
    (project.governance && project.governance.risk_level) || null;
  ...
  for (const card of cards) {
    ...
    const cardText = [
      ...
      fields.scope,
      fields.out_of_scope,
      ...stringList(fields.applies_when),
      ...stringList(fields.does_not_apply_when),
    ]
      .filter(Boolean)
      .join(" ")
      .toLowerCase();
...
function validateGovernance(project) {
  ...
  if (!gov.risk_level) {
    issues.push({
      ...
      severity: "blocking",
      message: "Governance: risk_level must be declared (R0/R1/R2/R3)",
    });
  }
  if (!gov.intended_use || !gov.intended_use.length) {
    issues.push({
      type: "missing_intended_use",
      severity: "blocking",
      message: "Governance: intended_use must be declared",
    });
  }
  if (!gov.out_of_scope || !gov.out_of_scope.length) {
    issues.push({
      ...
      severity: "blocking",
      message: "Governance: out_of_scope must be declared",
    });
  }
  if (!gov.known_limitations || !gov.known_limitations.length) {
    issues.push({
      ...
      severity: "blocking",
      message: "Governance: known_limitations must be declared",
    });
  }
  ...
  if (requiresExpertReview(riskLevel)) {
    if (!gov.reviewed_by) {
      issues.push({
        type: "requires_expert_review",
        severity: "blocking",
        message: `Governance: risk_level ${riskLevel} requires expert_review. reviewed_by must be set.`,
      });
    }
    if (!gov.risk_warnings || !gov.risk_warnings.length) {
      issues.push({
        type: "missing_risk_warnings",
        severity: "blocking",
        message: `Governance: risk_level ${riskLevel} requires risk_warnings.`,
      });
    }
  }
  ...
  if (
    gov.risk_level &&
    ["R0", "R1"].includes(gov.risk_level) &&
    ["R2", "R3"].includes(detectedLevel)
  ) {
    issues.push({
      type: "risk_mismatch",
      severity: "blocking",
  ...
  if (["R1", "R2", "R3"].includes(riskLevel) && !gov.author_statement) {
    issues.push({
      type: "missing_author_statement",
      severity: "blocking",
      message: `Governance: risk_level ${riskLevel} requires author_statement.`,
    });
  }
...
function generateKdnaCard(project, compiledStats, provenance, gates) {
  ...
  const lockedCards = cards.filter(
    (c) => c.locked && c.human_lock?.by && c.human_lock?.statement,
  );
  ...
  const card = {
    ...
    human_lock_summary: {
      ...
      locked_misunderstandings: lockedCards.filter(
        (c) => c.type === "misunderstanding",
      ).length,
      locked_self_checks: lockedCards.filter((c) => c.type === "self_check")
        .length,
      feynman_restatements: lockedCards.filter((c) => c.feynman_restatement)
        .length,
      locked_by: (project.author && project.author.id) || "unknown",
    },
    quality_badge:
      compiledStats && compiledStats.quality_gate_passed === true
        ? "tested"
        : "untested",
  ...
  if (gates) {
    ...
    if (!sagObj) {
      ...
      const currentHighest = sources.filter(
        (s) => s && typeof s === "object" && s.type === "human_locked_charter",
      );
      const authorConfirm = sources.filter(
        (s) => s && typeof s === "object" && s.type === "author_confirmation",
      );
      // PR-4b synthesis is detected via tc_status, not per-source flags
      const wasSynthesized =
        tcObj && tcObj.tc_status_before_lock === "synthesized";
      const migrated =
        project.migration && project.migration.synthesized === true;

      if (currentHighest.length > 0) {
        card.authority_status =
          wasSynthesized || migrated
    ...
    const migrationOverride =
      (project.migration && project.migration.status) || null;
    ...
    if (sagObj) {
      const sources = Array.isArray(sagObj.sources) ? sagObj.sources : [];
      const currentHighestCount = sources.filter(
        (s) => s && typeof s === "object" && s.type === "human_locked_charter",
      ).length;
      card.sag_summary = {
        sag_id:
          sagObj.id ||
          `sag_${project.name}_${new Date().toISOString().slice(0, 10)}`,
        version_intent: sagObj.version_intent || card.version,
        source_count: sources.length,
        current_highest_count: currentHighestCount,
        has_conflict_policies: !!(
          sagObj.conflict_policies &&
          Object.keys(sagObj.conflict_policies).length > 0
        ),
        sensitivity: {
          pii: !!(sagObj.sensitivity && sagObj.sensitivity.sources_contain_pii),
          author_consent_on_file: !!(
            sagObj.sensitivity && sagObj.sensitivity.author_consent_on_file
          ),
        },
      };
    }
    ...
    if (tcObj) {
      card.tc_summary = {
        tc_id:
          tcObj.id ||
          `tc_${project.name}_${card.version}_${new Date().toISOString().slice(0, 10)}`,
        highest_question: tcObj.highest_question || "",
        in_scope_count: Array.isArray(tcObj.in_scope)
          ? tcObj.in_scope.length
          : 0,
        out_of_scope_count: Array.isArray(tcObj.out_of_scope)
          ? tcObj.out_of_scope.length
          : 0,
        renamed_terms_count: Array.isArray(tcObj.renamed_terms)
          ? tcObj.renamed_terms.length
          : 0,
        highest_axiom_protected_chars:
          typeof tcObj.highest_axiom_protected === "string"
            ? tcObj.highest_axiom_protected.length
            : 0,
      };
    }
    ...
    if (manifest) {
      ...
      card.module_summary = {
        ...
        internal_module_count: modules.filter(
          (m) => m && m.type === "internal_module",
        ).length,
        sub_domain_count: modules.filter((m) => m && m.type === "sub_domain")
          .length,
        reference_count: modules.filter((m) => m && m.type === "reference")
          .length,
        decomposition_rationale_present:
          typeof manifest.decomposition_rationale === "string" &&
