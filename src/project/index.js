...
function loadProject(json) {
  ...
  if (!result.valid) {
    throw new Error(
      "loadProject: project validation failed:\n  - " +
        result.issues.join("\n  - "),
    );
  }
  return project;
...
function validateProject(project) {
  ...
  function checkType(val, expected, path) {
    if (expected === "string" && typeof val !== "string")
      issues.push(path + ": expected string, got " + typeof val);
    else if (expected === "number" && typeof val !== "number")
      issues.push(path + ": expected number, got " + typeof val);
    else if (
      expected === "integer" &&
      (!Number.isInteger(val) || typeof val !== "number")
    )
      issues.push(path + ": expected integer, got " + typeof val);
    else if (expected === "boolean" && typeof val !== "boolean")
      issues.push(path + ": expected boolean, got " + typeof val);
    else if (
      expected === "object" &&
      ...
      issues.push(path + ": expected array, got " + typeof val);
  }
  ...
  if (project.studio_version !== undefined) {
    ...
    if (typeof project.studio_version === "string") {
      ...
      if (verPattern && !new RegExp(verPattern).test(project.studio_version)) {
        issues.push(
          "studio_version: invalid version format, expected semver (e.g. 1.2.3)",
        );
      }
    }
  }
  ...
  if (project.type !== undefined) {
    ...
    if (
      ...
      !["domain", "cluster"].includes(project.type)
    ) {
      issues.push(
        'type: must be "domain" or "cluster", got "' + project.type + '"',
      );
    }
  }
  ...
  // created / updated
  if (project.created !== undefined)
    checkType(project.created, "string", "created");
  if (project.updated !== undefined)
    checkType(project.updated, "string", "updated");
    ...
    if (Array.isArray(project.cards)) {
      for (let i = 0; i < project.cards.length; i++) {
        ...
        for (const req of ["id", "type", "status"]) {
          if (!(req in card))
            issues.push(
              "cards[" + i + ']: missing required field "' + req + '"',
            );
        }
        if (card.type !== undefined) {
          if (!CARD_TYPES.includes(card.type))
            issues.push("cards[" + i + ']: invalid type "' + card.type + '"');
        }
        if (card.status !== undefined) {
          ...
          if (!validStates.includes(card.status))
            issues.push(
              "cards[" + i + ']: invalid status "' + card.status + '"',
            );
        }
      }
    }
    ...
    if (Array.isArray(project.evidence)) {
      for (let i = 0; i < project.evidence.length; i++) {
        ...
        for (const req of ["id", "type", "title"]) {
          if (!(req in ev))
            issues.push(
              "evidence[" + i + ']: missing required field "' + req + '"',
            );
        }
      }
    }
    ...
    if (Array.isArray(project.tests)) {
      for (let i = 0; i < project.tests.length; i++) {
        ...
        for (const req of ["id", "input"]) {
          if (!(req in t))
            issues.push(
              "tests[" + i + ']: missing required field "' + req + '"',
            );
        }
      }
    }
...
function upgradeProject(project, fromVersion, toVersion) {
  ...
  if (migrations[key]) {
    ...
    throw new Error(
      "upgradeProject: no migration path from " +
        fromVersion +
        " to " +
        toVersion,
    );
...
function checkHumanLockGate(project) {
  ...
  const lockedJudgmentCards = cards.filter(
    (c) =>
      JUDGMENT_CARD_TYPES.has(c.type) &&
      ["locked", "tested", "published"].includes(c.status),
  );
  if (
    lockedJudgmentCards.length === 0 &&
    cards.some((c) => JUDGMENT_CARD_TYPES.has(c.type))
  ) {
    issues.push({
      cardId: "(project)",
      type: "project",
      reason:
        "No judgment-class cards are approved. At least one axiom, boundary, or risk card must be reviewed before Studio export.",
    });
  }
...
function exportProject(project, options = {}) {
  ...
  if (gate.blocked && options.requireHumanLock && options.force) {
    ...
    project._human_lock_override = {
      ...
      blocked_issues: gate.issues.map((i) => ({
        cardId: i.cardId,
        reason: i.reason,
      })),
    };
  ...
  project.release.human_lock_gate_passed =
    !gate.blocked || Boolean(options.requireHumanLock && options.force);
