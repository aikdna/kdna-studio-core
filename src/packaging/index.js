...
function devBundleSource(domainDir, outputDir = null) {
  ...
  const result = execFileSync("kdna", args, {
    encoding: "utf8",
    timeout: 60000,
  });
  return {
...
function packEncryptedDomain(domainDir, licensePath, outputDir = null) {
  ...
  return {
    success: false,
    error:
      "Encrypted-extension packaging has been removed. Build a licensed .kdna asset and activate it through the entitlement API.",
  };
}

function verifyDomain(domainPath) {
  try {
    const result = execFileSync("kdna", ["verify", domainPath, "--json"], {
      encoding: "utf8",
      timeout: 30000,
    });
...
function validateDomain(domainPath) {
  try {
    const result = execFileSync("kdna", ["dev", "validate", domainPath], {
      encoding: "utf8",
      timeout: 30000,
    });
...
function inspectContainer(filePath) {
  try {
    const result = execFileSync("kdna", ["inspect", filePath, "--json"], {
      encoding: "utf8",
      timeout: 15000,
    });
    return JSON.parse(result);
...
function signDomain(domainDir) {
  try {
    const result = execFileSync("kdna", ["publish", "--check", domainDir], {
      encoding: "utf8",
      timeout: 30000,
    });
    return { success: true, output: result.trim() };
