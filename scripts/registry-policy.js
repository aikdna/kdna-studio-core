...
function evaluateRegistryResult(result, rawEvidence) {
  ...
  assert(
    ...
    "registry lookup did not return an exit status",
  );
  ...
  if (result.status === 1) {
    ...
    assert(
      document.error.code === expected.code,
      "registry E404 code mismatch",
    );
    assert(
      document.error.summary === expected.summary,
      "registry E404 summary mismatch",
    );
    assert(
      document.error.detail === expected.detail,
      "registry E404 detail mismatch",
    );
  ...
  assert(
    result.status === 0,
    `registry lookup exited ${String(result.status)}`,
  );
  const metadata = parseJson(stdout, "registry metadata stdout");
  exactKeys(
    metadata,
    ["name", "version", "dist.integrity", "dist.shasum"],
    "registry metadata",
  );
  assert(
