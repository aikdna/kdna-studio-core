...
function defaultTrustedNpmTarball() {
  return path.join(
    os.tmpdir(),
    "aikdna-trusted-tools",
    `npm-${TRUSTED_NPM_VERSION}.tgz`,
  );
}
...
function canonicalizeParent(file) {
  ...
  while (!fs.existsSync(ancestor)) {
    ...
    assert.notEqual(
      parent,
      ancestor,
      "trusted npm release path has no existing ancestor",
    );
  ...
  assert.ok(
    stat.isDirectory(),
    "trusted npm release parent must resolve through a directory",
  );
...
function trustedTarballPath(
  explicitPath = process.env[TRUSTED_NPM_ENVIRONMENT],
) {
...
function verifyTrustedNpmTarball(file) {
  ...
  const stat = assertRegularCanonicalFile(
    canonical,
...
function tarOctal(block, offset, length, label) {
  ...
  assert.ok(
    ...
    `trusted npm tar ${label} is invalid`,
  );
  return value;
}
...
function readTrustedNpmEntries(bytes) {
  ...
  while (offset + 512 <= archive.length) {
    ...
    assert.equal(
      storedChecksum,
      actualChecksum,
      "trusted npm tar header checksum mismatch",
    );
    ...
    assert.ok(
      type === 0 || type === 48,
      `trusted npm tar entry type is unsupported: ${entryPath}`,
    );
    ...
    assert.ok(
      ...
      `trusted npm tar entry is truncated: ${entryPath}`,
    );
    seen.add(entryPath);
    entries.push({
      path: entryPath,
  ...
  assert.equal(
    entries.length,
    1938,
    "trusted npm tar entry count is not the audited release",
  );
  return entries;
}
...
function extractTrustedNpmRelease(file) {
  ...
  const temporary = fs.mkdtempSync(
    path.join(fs.realpathSync(os.tmpdir()), "aikdna-trusted-npm-"),
  );
  let complete = false;
  try {
    ...
    assert.equal(
      manifest.name,
      "npm",
      "audited npm manifest package name is invalid",
    );
    assert.equal(
      manifest.version,
      TRUSTED_NPM_VERSION,
      "audited npm manifest version is invalid",
    );
    complete = true;
