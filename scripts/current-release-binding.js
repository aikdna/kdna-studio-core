...
function validateCurrentBinding({
  ...
  if (evidence.package.name !== context.name)
    throw new Error("release evidence name is stale");
  if (evidence.package.version !== context.version)
    throw new Error("release evidence version is stale");
  if (evidence.source.ref !== context.ref)
    throw new Error("release evidence ref is stale");
  if (evidence.source.commit !== context.commit)
    throw new Error("release evidence commit is stale");
  return evidence;
}
