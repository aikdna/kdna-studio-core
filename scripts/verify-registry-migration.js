...
function installRegistryPackages(consumerRoot) {
  ...
  if (result.signal)
    throw new Error(`registry install was interrupted by ${result.signal}`);
  if (result.status !== 0) {
    throw new Error(
      `registry install failed with exit status ${String(result.status)}`,
    );
  }
}
...
function verifyCurrentMain(consumerRequire) {
  ...
  const project = studio.project.createProject(
    "registry_migration_smoke",
    "domain",
    {
      author: { name: "Registry Smoke", id: "registry_smoke" },
    },
  );
  const draft = studio.cards.createCard(
    "axiom",
    {
      one_sentence: "Runtime packages require explicit migration.",
      full_statement:
        "Registry migration must preserve the current authoring and runtime export contract.",
      ...
      failure_risk:
        "Consumers may assume compatibility that the registry artifacts do not provide.",
    },
    "ax_registry_migration",
  );
  const revised = studio.cards.transitionCard(draft, "revised", {
    by: "registry_smoke",
  });
...
function verifyLockfile(consumerRoot) {
  ...
  for (const [name, version] of [
    ...
    const matches = Object.entries(lock.packages).filter(
      ([key]) => key === `node_modules/${name}` || key.endsWith(suffix),
    );
    assert.ok(
      matches.length === 1,
...
function verifyInstalledMigration(consumerRoot) {
  const consumerRequire = createRequire(
    path.join(consumerRoot, "registry-consumer.cjs"),
  );
  const legacyManifest = verifyExactPackage(
    consumerRequire,
    LEGACY_NAME,
    LEGACY_VERSION,
  );
  const currentManifest = verifyExactPackage(
    consumerRequire,
    CURRENT_NAME,
    CURRENT_VERSION,
  );
...
function main() {
  const consumerRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), "kdna-studio-registry-migration-"),
  );
  try {
    installRegistryPackages(consumerRoot);
    verifyInstalledMigration(consumerRoot);
    console.log(
      `Verified registry migration boundary: ${LEGACY_NAME}@${LEGACY_VERSION} -> ${CURRENT_NAME}@${CURRENT_VERSION}`,
    );
  } finally {
