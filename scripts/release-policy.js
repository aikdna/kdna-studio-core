...
function validateReleaseContext({ pkg, changelog, env, git }) {
  ...
  assert(
    ...
    "package version must be stable canonical SemVer",
  );
  ...
  assert(
    ...
    "GITHUB_EVENT_NAME must be release",
  );
  assert(
    env.RELEASE_EVENT_ACTION === "published",
    "release action must be published",
  );
  ...
  assert(
    ...
    `CHANGELOG must contain one ## ${version}`,
  );
  const finalized = [
    ...changelog.matchAll(/^## (\d+\.\d+\.\d+)(?: \(\d{4}-\d{2}-\d{2}\))?$/gm),
  ];
  assert(
    finalized[0]?.[1] === version,
