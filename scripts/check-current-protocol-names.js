...
function isTextFile(relative) {
  if (["LICENSE", "NOTICE", "mimetype"].includes(path.basename(relative)))
    return true;
...
function readAllowlist(root) {
  ...
  if (!Array.isArray(entries))
    throw new Error("third-party naming allowlist must be an array");
...
function removeAllowedText(relative, text, allowlist) {
  ...
  for (const entry of allowlist.filter(
    (candidate) => candidate.file === relative,
  )) {
...
function scanPackedArtifact(root) {
  const temporary = fs.mkdtempSync(
    path.join(os.tmpdir(), "kdna-studio-core-names-"),
  );
  ...
  try {
    ...
    for (const entry of readTarFileEntries(tarball)) {
      ...
      if (!isTextFile(relative)) continue;
      findings.push(
        ...findingsForText(relative, entry.bytes.toString("utf8"), []),
      );
    }
...
function main() {
  ...
  if (findings.length > 0) {
    ...
    throw new Error(
      `current protocol naming gate found ${findings.length} issue(s)`,
    );
  }
