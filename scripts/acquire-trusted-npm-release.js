...
function destinationFromArguments(argv) {
  ...
  assert.equal(
    argv.length,
    2,
    "usage: acquire-trusted-npm-release.js [--out <tarball>]",
  );
  assert.equal(
    argv[0],
    "--out",
...
function download() {
  return new Promise((resolve, reject) => {
    const request = https.get(
      ...
      (response) => {
        if (response.statusCode !== 200) {
          ...
          reject(
            new Error(
              `trusted npm release download returned HTTP ${String(response.statusCode)}`,
            ),
          );
          return;
        }
        if (response.headers.location || response.headers["content-encoding"]) {
          ...
          reject(
            new Error(
              "trusted npm release download response was not canonical raw bytes",
            ),
          );
          return;
        }
        ...
        response.on("data", (chunk) => {
          ...
          if (size > MAX_DOWNLOAD_BYTES) {
            request.destroy(
              new Error("trusted npm release download exceeded its size limit"),
            );
            return;
          }
          chunks.push(chunk);
    ...
    request.setTimeout(30_000, () =>
      request.destroy(new Error("trusted npm release download timed out")),
    );
...
if (require.main === module) {
  acquire(destinationFromArguments(process.argv.slice(2)))
    .then((destination) =>
      console.log(`Verified trusted npm release: ${destination}`),
    )
    .catch((error) => {
