...
const CANDIDATE_ARTIFACT_RE =
  /^fixtures\/runtime-candidates\/[a-z0-9][a-z0-9._-]*\.tgz$/;
...
function createTrustedNodeEntry(nodeExecPath) {
  ...
  try {
    if (process.platform === "win32") {
      ...
      assert(
        !nodeExecPath.includes('"'),
        "trusted Node executable path cannot contain quotes",
      );
...
function trustedNpmEnvironment({
  baseEnvironment,
  nodeExecPath,
  nodeEntry,
  npmCliPath,
  ...
  const executableDirectories = [
    nodeEntry.directory,
    path.dirname(resolveTrustedSystemGit()),
  ];
  if (process.platform !== "win32")
    executableDirectories.push("/usr/bin", "/bin");
...
function resolveTrustedNpmInvocation(root, options = {}) {
  ...
  assert(
    path.isAbsolute(nodeExecPath),
    "trusted Node executable path must be absolute",
  );
  ...
  assert(
    fs.realpathSync(nodeExecPath) === nodeExecPath,
    "trusted Node executable path must be canonical",
  );
...
function strictRegistryLookup(name, version, options = {}) {
  ...
  assert(
    result.stderr === "",
    "registry dependency lookup wrote unexpected stderr",
  );
  let metadata;
  try {
    ...
    throw new Error(
      "registry dependency lookup must return one complete JSON document",
    );
  }
  assert(
  ...
  assert(
    JSON.stringify(fields) ===
      JSON.stringify(["dist.integrity", "name", "version"]),
...
function assertAuthorityFile(file, expectedRealPath, label) {
  ...
  assert(
    fs.realpathSync(file) === expectedRealPath,
    `${label} escapes its canonical path`,
  );
}
...
function readTarFileEntriesFromBytes(compressed, label = "candidate tar") {
  ...
  while (offset + 512 <= archive.length) {
    ...
    assert(
      type === 0 || type === 48,
      `${label} entry type is unsupported: ${entryPath}`,
    );
    assert(
    ...
    assert(
      bodyEnd <= archive.length,
      `${label} entry is truncated: ${entryPath}`,
    );
    paths.add(entryPath);
    entries.push(
      Object.freeze({
...
function assertPackageTarInstallEquivalent(
  referenceBytes,
  candidateBytes,
  options = {},
) {
  ...
  const referenceByPath = new Map(
    referenceEntries.map((entry) => [entry.path, entry]),
  );
...
function tarPackageManifest(artifact) {
  ...
  assert(
    manifests.length === 1,
    "candidate tar package manifest is missing or duplicated",
  );
...
function assertDependencyMaps(
  container,
  label,
  directVersions,
  requireFormalMap = false,
) {
  for (const mapName of DEPENDENCY_MAP_NAMES) {
    for (const [name, spec] of Object.entries(container?.[mapName] || {})) {
      ...
      assert(
        nameReferences.length === 1 &&
          nameReferences[0] === name &&
          AIKDNA_PACKAGE_RE.test(name),
        `${label} AIKDNA package name invalid: ${name}`,
      );
      assert(
        directVersions.has(name),
        `${label} references undeclared AIKDNA package: ${name}`,
      );
      assert(
...
function assertExactPackageNames(label, actualNames, expectedNames) {
  ...
  assert(
    duplicates.length === 0,
    `${label} contains duplicate packages: ${duplicates.join(", ")}`,
  );
...
function parseLockPackagePath(lockPath) {
  ...
  while (index < segments.length) {
    ...
    if (first.startsWith("@")) {
      ...
      packages.push({
        name: `${first}/${leaf}`,
        lockPath: segments.slice(0, index).join("/"),
      });
    } else {
      index += 1;
      packages.push({
...
function assertExactAikdnaLockPackages(
  packageLock,
  directNames,
  boundNames,
  directVersions,
  ...
  for (const [lockPath, locked] of Object.entries(packageLock.packages || {})) {
    ...
    if (referencesAikdnaScope(lockPath)) {
      ...
      for (const { name, lockPath: packagePath } of packages.filter(
        ({ name }) => referencesAikdnaScope(name),
      )) {
        assert(
          AIKDNA_PACKAGE_RE.test(name),
    ...
    if (lockedNameReferences.length > 0) {
      ...
      assert(
        directSet.has(locked.name),
        `unbound AIKDNA lock package: ${locked.name}`,
      );
    }
...
function verifyCandidateBinding(root) {
  ...
  const expectedBindingReal = path.join(
    rootReal,
    ...pathSegments(BINDING_PATH),
  );
  ...
  const workflowPath = path.join(
    root,
    ...pathSegments(CANDIDATE_WORKFLOW_PATH),
  );
  assertAuthorityFile(
    packagePath,
    path.join(rootReal, "package.json"),
    "package manifest",
  );
  assertAuthorityFile(
    lockPath,
    path.join(rootReal, "package-lock.json"),
    "package lock",
  );
  assertAuthorityFile(
    workflowPath,
  ...
  for (const authority of CANDIDATE_AUTHORITIES) {
    const evidencePath = path.join(
      root,
      ...pathSegments(authority.evidencePath),
    );
    assertAuthorityFile(
      evidencePath,
  ...
  assert(
    ...
    "candidate binding schema version mismatch",
  );
  assert(
  ...
  const directNames = aikdnaDependencyNames(
  ...
  for (const name of directNames) {
    ...
    assert(
      SEMVER_RE.test(declared || ""),
      `direct dependency must use exact SemVer: ${name}`,
    );
    assert(
    ...
    assert(
      ...
      `lock package version mismatch: ${name}`,
    );
    assert(
      INTEGRITY_RE.test(locked.integrity || ""),
      `lock package integrity invalid: ${name}`,
    );
  ...
  for (const entry of binding.packages) {
    ...
    const candidateDirectory = path.join(
      root,
      ...pathSegments(candidateDirectoryRelative),
    );
    const expectedDirectoryReal = path.join(
      rootReal,
      ...pathSegments(candidateDirectoryRelative),
    );
    let directoryStat;
    ...
    const expectedArtifactReal = path.join(
      rootReal,
      ...pathSegments(entry.artifact),
    );
    assertAuthorityFile(
      artifact,
      expectedArtifactReal,
      `candidate artifact ${entry.name}`,
    );
    ...
    assert(
      candidateEvidence.pack?.status &&
        isValidCandidatePackStatus(candidateEvidence.pack.status) &&
        candidateEvidence.pack?.npm_client === TRUSTED_NPM_VERSION &&
        candidateEvidence.pack?.npm_release_url === TRUSTED_NPM_URL &&
        candidateEvidence.pack?.npm_release_integrity ===
          TRUSTED_NPM_INTEGRITY &&
    ...
    assert(
      ...
      `candidate tar package name mismatch: ${entry.name}`,
    );
    assert(
...
function verifyInstalledAikdnaGraph(root) {
  ...
  function scanForNestedNodeModules(directory) {
    for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
      ...
      if (entry.isSymbolicLink()) {
        throw new Error(
          `installed package tree contains a symlink: ${entryPath}`,
        );
      }
  ...
  function scanNodeModules(nodeModules) {
    ...
    for (const entry of fs.readdirSync(nodeModules, { withFileTypes: true })) {
      ...
      if (entry.name === ".bin") {
        ...
        for (const executable of fs.readdirSync(entryPath, {
          withFileTypes: true,
        })) {
          ...
          if (executable.isSymbolicLink()) {
            ...
            try {
              ...
              throw new Error(
                `.bin contains a broken symlink: ${executablePath}`,
              );
            }
            assert(
            ...
            assert(
              executable.isFile(),
              `.bin must not contain directories: ${executablePath}`,
            );
          }
        }
        continue;
      ...
      if (entry.isSymbolicLink()) {
        throw new Error(
          `installed package graph contains a symlink: ${entryPath}`,
        );
      }
      if (!entry.isDirectory()) continue;
  ...
  return Object.freeze(
    Object.fromEntries(
      directNames.map((name) => [name, directVersions.get(name)]),
    ),
  );
}
...
function assertRegistryReleaseReady(root, registryLookup = null) {
  ...
  for (const entry of binding.packages) {
    ...
    assert(
      ...
      `registry package name mismatch: ${entry.name}`,
    );
    assert(
      metadata.version === entry.version,
      `registry package version mismatch: ${entry.name}`,
    );
    assert(
