...
function resolveTrustedSystemGit() {
    ...
    try {
      ...
      if (process.platform !== "win32") {
        ...
        assert.equal(
          stat.mode & 0o022,
          0,
          "trusted Git must not be group- or world-writable",
        );
      }
      ...
      assert.equal(
        result.error,
        undefined,
        "trusted Git version probe failed to start",
      );
      assert.equal(
        result.signal,
        null,
        "trusted Git version probe was interrupted",
      );
      assert.equal(result.status, 0, "trusted Git version probe failed");
      assert.equal(
        result.stderr,
        "",
        "trusted Git version probe wrote unexpected stderr",
      );
      assert.match(
        result.stdout.trim(),
        /^git version \d+(?:\.\d+)+(?:[.\s-].*)?$/,
  ...
  throw new Error(
    `no trusted system Git executable is available (${failures.join("; ")})`,
  );
}
...
function authoritativeGit(repository, args, options = {}) {
  ...
  assert.equal(
    result.error,
    undefined,
    "authoritative Git command failed to start",
  );
  assert.equal(
    result.signal,
    null,
    "authoritative Git command was interrupted",
  );
...
function materializeCommitTree(
  ...
  while (recordStart < listing.length) {
    ...
    assert.notEqual(
      recordEnd,
      -1,
      "authoritative commit tree listing is not NUL terminated",
    );
    assert.ok(
      recordEnd > recordStart,
      "authoritative commit tree listing contains an empty record",
    );
    records.push(listing.subarray(recordStart, recordEnd));
    recordStart = recordEnd + 1;
  }
  assert.ok(
    records.length > 0 && records.length <= 20_000,
    "authoritative commit tree entry count is invalid",
  );
  ...
  for (const record of records) {
    ...
    assert.ok(
      separator > 0,
      "authoritative commit tree record has no path separator",
    );
    const header = record.subarray(0, separator).toString("ascii");
    const match =
      /^(040000|100644|100755|120000|160000) (tree|blob|commit) ([0-9a-f]{40})$/.exec(
        header,
      );
    ...
    assert.ok(
      Buffer.from(canonicalEntry, "utf8").equals(
    ...
    assert.ok(
      isWithinPrefix,
      `authoritative commit tree escaped its source prefix: ${canonicalEntry}`,
    );
    assert.ok(
    ...
    if (mode === "040000") {
      assert.equal(
        type,
        "tree",
        `authoritative commit directory type is invalid: ${canonicalEntry}`,
      );
      if (stripped) {
      ...
      assert.ok(
        stripped,
        "authoritative source prefix must resolve to a directory",
      );
      const output = path.join(destinationRoot, ...stripped.split("/"));
      ...
      assert.ok(
        ...
        "authoritative commit tree is too large",
      );
  ...
  if (normalizedPrefix) {
    assert.ok(
      seen.has(normalizedPrefix),
      "authoritative source tree root is missing",
    );
  }
  ...
  assert.ok(
    seen.has(requiredPath),
    `authoritative required file is missing: ${requiredPath}`,
  );
