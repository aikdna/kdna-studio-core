...
function readPinnedCandidateCommits(root) {
  ...
  return new Map(
    CANDIDATE_AUTHORITIES.map((authority) => {
      const escapedRepositoryEnvironment =
        authority.repositoryEnvironment.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
      const escapedCommitEnvironment = authority.commitEnvironment.replace(
        /[.*+?^${}()|[\]\\]/g,
        "\\$&",
      );
      const escapedRepository = authority.repository.replace(
        /[.*+?^${}()|[\]\\]/g,
        "\\$&",
      );
      ...
      const refs = [...workflow.matchAll(pattern)].map((match) =>
        match[1].toLowerCase(),
      );
