...
function parseTarNumber(field) {
  if (field[0] & 0x80) {
    ...
    assert(
      ...
      "tar entry size exceeds the safe range",
    );
    return Number(value);
  }
...
function parsePax(buffer) {
  ...
  while (offset < buffer.length) {
    ...
    const record = buffer
      ...
      .toString("utf8");
    assert(
      buffer[offset + length - 1] === 0x0a,
      "PAX record must end with a newline",
    );
...
function packagePathFromTar(name) {
  assert(
    typeof name === "string" && name.startsWith("package/"),
    "packed entry must be rooted under package/",
  );
...
function parseTarFiles(tarball) {
  assert(
    Buffer.isBuffer(tarball) && tarball.length > 0,
    "packed tarball is empty",
  );
  let archive;
  try {
  ...
  while (offset + 512 <= archive.length) {
    ...
    if (header.every((byte) => byte === 0)) {
      ...
      assert(
        second.length === 512 && second.every((byte) => byte === 0),
        "tar archive has an incomplete end marker",
      );
      assert(
    ...
    assert(
      dataEnd <= archive.length && nextOffset <= archive.length,
      "truncated tar entry",
    );
    ...
    if (type === "x") {
      ...
      assert(
        effectiveSize === size,
        "PAX file size does not match the tar entry",
      );
      if (type === "0" || type === "\0") {
        files.push({
          path: packagePathFromTar(effectiveName),
          size: effectiveSize,
        });
      } else if (type === "5") {
  ...
  assert(
    Object.keys(pax).length === 0 && longName === null,
    "tar metadata record has no target entry",
  );
  files.sort((left, right) => left.path.localeCompare(right.path));
  assert(
    new Set(files.map((file) => file.path)).size === files.length,
    "tar archive contains duplicate file paths",
  );
...
function validatePackReport({ reportText, tarball, pkg, source }) {
  ...
  assert(
    STABLE_VERSION_RE.test(pkg.version || ""),
    "npm pack package version is invalid",
  );
  assert(
    source.ref === `refs/tags/${pkg.version}`,
    "npm pack source ref mismatch",
  );
  assert(
    COMMIT_RE.test(source.commit || ""),
    "npm pack source commit is invalid",
  );
  const reports = parseJsonDocument(reportText, "npm pack output");
  assert(
    Array.isArray(reports) && reports.length === 1,
    "npm pack must report one artifact",
  );
  const report = reports[0];
  assert(
    report.name === pkg.name && report.version === pkg.version,
  ...
  assert(
    ...
    "npm pack entry count does not match the tarball",
  );
  assert(
    report.unpackedSize === unpackedSize,
    "npm pack unpacked size does not match the tarball",
  );
  return {
...
function validateEvidence(evidence) {
  assert(
    evidence?.schema === "kdna.studio-core.release-evidence",
    "release evidence schema mismatch",
  );
  assert(evidence.version === "1.0", "release evidence version mismatch");
  assert(
    evidence.package?.name === EXPECTED_PACKAGE_NAME,
    "release evidence package mismatch",
  );
  assert(
    STABLE_VERSION_RE.test(evidence.package.version || ""),
    "release evidence package version invalid",
  );
  assert(
    evidence.source?.ref === `refs/tags/${evidence.package.version}`,
    "release evidence ref mismatch",
  );
  assert(
    COMMIT_RE.test(evidence.source.commit || ""),
    "release evidence commit invalid",
  );
  assert(
  ...
  assert(
    isCanonicalSha512Integrity(evidence.artifact?.integrity),
    "release evidence integrity invalid",
  );
  assert(
    /^[0-9a-f]{40}$/.test(evidence.artifact?.shasum || ""),
    "release evidence shasum invalid",
  );
  assert(
    Number.isSafeInteger(evidence.artifact?.packed_size) &&
      evidence.artifact.packed_size > 0,
    "release evidence packed size invalid",
  );
  assert(
    Number.isSafeInteger(evidence.artifact?.unpacked_size) &&
      evidence.artifact.unpacked_size > 0,
    "release evidence unpacked size invalid",
  );
  assert(
    Number.isSafeInteger(evidence.artifact?.file_count) &&
      evidence.artifact.file_count > 0,
    "release evidence file count invalid",
  );
  const files = validateFiles(evidence.artifact.files);
  assert(
    files.length === evidence.artifact.file_count,
    "release evidence files mismatch",
  );
  assert(
    files.reduce((total, file) => total + file.size, 0) ===
      evidence.artifact.unpacked_size,
...
function validateArtifact(rawEvidence, tarball) {
  ...
  assert(
    ...
    "verified artifact size mismatch",
  );
  assert(
    integrity(tarball) === evidence.artifact.integrity,
    "verified artifact integrity mismatch",
  );
  assert(
    sha1(tarball) === evidence.artifact.shasum,
    "verified artifact shasum mismatch",
  );
  const files = parseTarFiles(tarball);
  const evidenceFiles = validateFiles(evidence.artifact.files);
  assert(
    JSON.stringify(files) === JSON.stringify(evidenceFiles),
    "verified artifact files mismatch",
  );
  assert(
    files.length === evidence.artifact.file_count,
    "verified artifact file count mismatch",
  );
  assert(
    files.reduce((total, file) => total + file.size, 0) ===
      evidence.artifact.unpacked_size,
