...
- Mark the retired fixed SAG/TC quality-gate design as historical; current
  Creation acceptance uses independent format, judgment and risk-adaptive
  application gates.
- Add the unreleased UI-independent Creation Engine for natural-language Host
  integration, content-free source inventory, recoverable source review,
  judgment modeling, semantic tests, repair, managed candidate generation, and
  final exact-byte delivery.
- Keep execution and authority as two explicit axes. Human participation is a
  receipt rather than an authority mode, and private authority evidence never
  enters Runtime.
- Bind material approval to exact inventory bytes, a local or named-remote
  processing destination, and an honest assurance level. Caller-declared
  capability digests are not verified-local proof.
- Separate format validity, judgment acceptance, and application verification while binding each receipt to current semantic, build, asset, and operation identities.
- Add optimistic concurrency and revision-bound mutations so stale or conflicting Agent operations fail closed.
- Support autonomous Agent-authored and independently evaluated interpretive
  workflows without inventing human or organization confirmation.
  Representational and mixed-authorship claims retain their exact authority
  boundaries.
- Derive semantic and application coverage from declared structure and risk.
  A narrow one-unit asset can remain narrow; declared high-risk or unique
  judgments and relations fail closed when uncovered.
- Require repeated, receipt-bound evidence for at least one pre-frozen core or
  highest-risk application scenario while avoiding a fixed repeat count for
  every task.
- Preserve public KDNA Manifest, Payload, LoadPlan, and Runtime Capsule contracts without adding private Creation evidence fields.
