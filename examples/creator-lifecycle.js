...
console.log(
  "2. AUTHOR — Create judgment cards (human, agent, tool, or hybrid)",
);
const axiom = createCard("axiom", {
  one_sentence: "Most writing problems are structural, not language-level.",
  full_statement:
    "When reviewing content, diagnose structure before language. Surface polishing on structurally weak content wastes effort.",
  why: "Without this principle, agents default to surface-level editing.",
  applies_when: [
    "User asks to review content",
    "User submits a draft",
    "User asks whether writing is good",
  ],
  does_not_apply_when: [
    "User explicitly asks for grammar check only",
    "User asks for translation",
  ],
  failure_risk:
    "May over-diagnose structural problems in content that only needs language polish.",
});
...
const misunderstanding = createCard("misunderstanding", {
  ...
  correct:
    "Good writing means having a clear argument, specific evidence, and a hook that captures attention.",
...
const selfCheck = createCard("self_check", {
  question:
    "Before suggesting language changes, did I verify the content has a clear argument and sufficient evidence?",
});
...
console.log(
  "3. REVIEW — Optional Human Lock provenance for reviewed publishing",
);

// Check gate BEFORE locking
const gateBefore = checkHumanLockGate(project);
console.log(
  `   Gate check (before lock): ${gateBefore.blocked ? "❌ BLOCKED" : "✅ PASS"}`,
);
...
console.log(
  `   Gate check (after lock):  ${gateAfter.blocked ? "❌ BLOCKED" : "✅ PASS"}`,
);
...
try {
  ...
  console.log(
    `   Reviewed:   ${compiled.stats.human_lock_count} cards with Human Lock`,
  );
  console.log(
    `   Excluded:   ${compiled.stats.excluded_cards} incomplete/deprecated cards`,
  );
  console.log("");
...
try {
  ...
  console.log(
    "   (Studio review provenance is separate from KDNA format validity)",
  );
}
console.log("");
...
console.log(
  "  Human Lock is optional provenance for reviewed publishing flows.",
);
console.log("═".repeat(60));
