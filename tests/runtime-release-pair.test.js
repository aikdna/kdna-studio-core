...
test("release dependency graph binds the exact Core candidate and has no Runtime CLI dependency", () => {
  ...
  assert.equal(
    packageLock.packages[""].dependencies["@aikdna/kdna-core"],
    "0.21.0",
  );
  assert.equal(
    packageLock.packages[""].dependencies["@aikdna/kdna-cli"],
    undefined,
  );
  assert.equal(
    packageLock.packages["node_modules/@aikdna/kdna-core"].version,
    "0.21.0",
  );
  assert.equal(
    packageLock.packages["node_modules/@aikdna/kdna-cli"],
    undefined,
  );
...
test("blank authoring project reaches current Core LoadPlan and Runtime Capsule", (t) => {
  ...
  project.cards.push(
    createCard(
      ...
      {
        one_sentence:
          "A portable judgment must preserve its declared boundary.",
        full_statement:
  ...
  const directory = fs.mkdtempSync(
    path.join(os.tmpdir(), "kdna-studio-release-pair-"),
  );
  ...
  assert.deepEqual(capsule.context.axioms[0].applies_when, [
    "Exporting a runtime asset",
  ]);
  assert.deepEqual(capsule.context.axioms[0].does_not_apply_when, [
    "Editing authoring notes",
  ]);
