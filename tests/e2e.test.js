...
function createFullProject() {
  ...
  const ax1 = makeLockedCard(
    "axiom",
    {
      ...
      failure_risk:
        "May cause over-scrutiny of decision quality when issue is resource availability.",
    },
    "ax_001",
  );
  project.cards.push(ax1);

  const ax2 = makeLockedCard(
    "axiom",
    {
      one_sentence:
        "Broad agreement is not commitment. Only named ownership is commitment.",
      full_statement:
        "A decision without a single named owner with a deadline has no commitment. Multiple owners = no owner.",
      why: 'Teams confuse "everyone nodding" with actual commitment. Without a named owner, no one wakes up responsible.',
      applies_when: [
        'Meeting ends with "sounds good"',
        "Decision without deadline",
        "Multiple people assigned",
      ],
      does_not_apply_when: [
        "Solo work with clear self-accountability",
        "Informal team alignment",
      ],
  ...
  const ax3 = makeLockedCard(
    "axiom",
    {
      one_sentence:
        "The cost of a slow decision is usually higher than the cost of an imperfect decision.",
      full_statement:
        "In leadership, decision speed compounds. A two-week delay for a perfect decision often costs more than an 80% decision made today.",
      why: "Leaders who wait for perfect information create organizational bottlenecks.",
      applies_when: [
        "Decision is reversible",
        "Stakes are below team/org level",
        "More information unlikely to change outcome",
      ],
      does_not_apply_when: [
        "Safety-critical decisions",
        "Irreversible resource commitments",
        "Legal/compliance required decisions",
      ],
      failure_risk:
        "May encourage premature decisions in high-stakes, irreversible situations.",
    },
    "ax_003",
  );
  ...
  const ms1 = makeLockedCard(
    "misunderstanding",
    {
      ...
      key_distinction:
        "Motivation gaps produce gradual decline over weeks. Decision voids produce sudden stalls within days. The pattern is fundamentally different.",
    },
    "ms_001",
  );
  project.cards.push(ms1);

  const ms2 = makeLockedCard(
    "misunderstanding",
    {
      wrong: "Consensus means everyone agrees with the decision.",
      correct:
        "Consensus means everyone understands the decision, knows their role in executing it, and commits to not blocking it — even if they disagree.",
      key_distinction:
        "Agreement is an emotional state. Commitment to execute is a behavioral contract. You need the latter, not the former.",
    },
    "ms_002",
  );
...
describe("E2E: compile → validate", () => {
  ...
  test("compiled output structure matches KDNA SPEC", () => {
    ...
    assert.ok(
      Array.isArray(core.core_structure),
      "core_structure array required",
    );
    ...
    assert.ok(
      ...
      `file_count should be >= 2, got ${manifest.file_count}`,
    );

    // Verify KDNA_Reasoning.json (should exist since we have axioms)
    assert.ok(
      "KDNA_Reasoning.json" in result.files,
    ...
    assert.ok(
      reasoning.reasoning_chains.every((c) => "so_what" in c),
      "Every chain must have so_what",
    );
    assert.ok(
      reasoning.reasoning_chains.every((c) => "one_sentence" in c),
      "Every chain must have one_sentence",
    );

    // Verify KDNA_Evolution.json
    assert.ok(
      "KDNA_Evolution.json" in result.files,
      "Evolution file should exist",
    );
  ...
  test("compiled axioms have required fields", () => {
    ...
    for (const ax of core.axioms) {
      ...
      assert.ok(
        Array.isArray(ax.applies_when),
        `${ax.id}: applies_when must be array`,
      );
      assert.ok(
        Array.isArray(ax.does_not_apply_when),
        `${ax.id}: does_not_apply_when must be array`,
      );
  ...
  test("compiled misunderstandings have key_distinction", () => {
    ...
    for (const ms of patterns.misunderstandings) {
      ...
      assert.ok(
        ms.key_distinction.length >= 20,
        `${ms.id}: key_distinction too short`,
      );
    }
  });
});
