...
const STAGING_HOLDER_CHILD = path.join(
  __dirname,
  "identity-staging-holder-child.js",
);
...
function tempIdentityParent() {
  const parentDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "kdna-identity-parent-"),
  );
...
function stagingDirs(parentDir) {
  return fs
    ...
    .filter(
      (name) => name.startsWith(".kdna-init-") && name.endsWith(".staging.d"),
    );
}
...
async function crashInitAtPhase(phase, maxAttempts = 5) {
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    const baseDir = fs.mkdtempSync(
      path.join(os.tmpdir(), `kdna-crash-${phase}-`),
    );
    ...
    if (fs.existsSync(markerPath)) {
      assert.notEqual(
        result.code,
        0,
        `crash child at phase ${phase} exited cleanly`,
      );
  ...
  assert.fail(
    `crash harness never observed phase "${phase}" in ${maxAttempts} attempts`,
  );
  return null;
}
...
test("initIdentity: a non-directory ancestor reports ENOTDIR instead of unrelated EEXIST", () => {
  const root = fs.mkdtempSync(
    path.join(os.tmpdir(), "kdna-identity-parent-blocker-"),
  );
...
test("initIdentity: a pre-existing creator.json is never overwritten", () => {
  ...
  const preExisting = JSON.stringify(
    { creator_id: "kdna:creator:ed25519:pre-existing" },
    null,
    2,
  );
  fs.writeFileSync(path.join(dir, "creator.json"), preExisting, {
    mode: 0o644,
  });
...
for (const phase of ["key", "pub", "json"]) {
  test(`initIdentity: SIGKILL after the ${phase} write (pre-commit) never publishes, and the next init recovers without manual cleanup`, async () => {
    ...
    if (fs.existsSync(path.join(dir, "creator.json"))) {
      ...
      assert.equal(
        loaded.creator_id,
        creatorFingerprint(readKey(dir, "kdna.pub")),
      );
      assert.ok(
        verifySignature(
          "late-kill",
          readKey(dir, "kdna.pub"),
          signPayload("late-kill", dir),
        ),
      );
      return;
    }
...
test("initIdentity: four concurrent subprocesses yield exactly one identity and losers never damage the winner", async () => {
  ...
  const results = await Promise.all(
    Array.from({ length: 4 }, () => runNode(["-e", script])),
  );
...
test("initIdentity: a live owner's staging is never reclaimed at any age; a dead owner's provable remnant is", async (t) => {
  const parentDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "kdna-live-staging-"),
  );
  ...
  const stagingName = fs.readFileSync(
    path.join(parentDir, ".holder-ready"),
    "utf8",
  );
  ...
  try {
    fs.utimesSync(stagingDir, longAgo, longAgo);
  } catch {
    /* best effort */
  }
...
test("initIdentity: a parent-directory fsync failure after the commit rename reports a committed identity, never a plain failure", () => {
  ...
  assert.equal(
    JSON.stringify(failure).includes(readKey(dir, "kdna.key")),
    false,
  );
...
test("initIdentity: post-commit verification failure is committed-but-inconsistent and every use path fails closed", () => {
  ...
  fs.openSync = function openSync(target, flags, mode) {
    if (flags === "r" && target === parentDir) {
      ...
      fs.writeFileSync(
        path.join(dir, "creator.json"),
        "{ committed but corrupt",
      );
      throw injected;
    }
    return realOpenSync.call(fs, target, flags, mode);
...
test("initIdentity: a pre-commit failure publishes nothing and init is safely retryable", () => {
  ...
  fs.openSync = function openSync(target, flags, mode) {
    if (flags === "r" && path.basename(target).startsWith(".kdna-init-"))
      throw injected;
    return realOpenSync.call(fs, target, flags, mode);
  };
  let failure = null;
...
test("cross-directory tamper: the kdna.pub of identity B inside directory A is rejected by load, loadPublicKey, and sign", () => {
  ...
  fs.writeFileSync(path.join(dirA, "kdna.pub"), readKey(dirB, "kdna.pub"), {
    mode: 0o644,
  });
...
test("signPayload/signHumanLock: signatures verify against the stored public key", () => {
  ...
  assert.equal(
    crypto.verify(null, Buffer.from(lockPayload), publicKeyPem, lockSigBytes),
    true,
  );
});
...
test("encryptPrivateKey: new envelopes are written at 600000 PBKDF2 iterations", () => {
  ...
  assert.ok(
    verifySignature(
      "encrypted-sign",
      loadPublicKey(dir),
      signPayload("encrypted-sign", dir, "passphrase"),
    ),
  );
});
...
test("encryptPrivateKey: KDF failures carry a stable machine-readable code", () => {
  ...
  try {
    const failure = captureError(() =>
      encryptPrivateKey("private-key", "passphrase"),
    );
