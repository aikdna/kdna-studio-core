...
test("lockCard sets human_lock and transitions to locked", () => {
  let card = createCard("axiom", {
    one_sentence: "Test",
    full_statement:
      "A complete testable explanation with enough detail for the agent.",
    why: "Without this the agent would make wrong calls.",
  });
...
test("lockCard rejects missing checked fields", () => {
  ...
  card = transitionCard(card, "revised", { by: "test_user" });
  assert.throws(
    () => lockCard(card, { by: "u", statement: "ok", checked: {} }),
    /applies_when/,
  );
});
...
test("getLockedCards filters correctly", () => {
  ...
  let c1 = createCard("axiom", {
    full_statement: "A complete testable explanation for the agent to use.",
    why: "This prevents the agent from making wrong calls.",
  });
...
test("compileDomain includes non-deprecated cards and reports review provenance separately", () => {
  ...
  let card = createCard("axiom", {
    ...
    full_statement:
      "A complete testable explanation of the judgment principle.",
...
test("compileDomain preserves explicit AI authorship without inventing human provenance", () => {
  ...
  project.cards = [
    createCard("axiom", {
      ...
      full_statement:
        "A valid KDNA asset may be authored by AI without being mislabeled as human-authored.",
  ...
  assert.equal("KDNA_CARD.json" in result.files, false);
  assert.equal(
    result.files["KDNA_Evolution.json"].includes("Human judgment confirmed"),
    false,
  );
});
...
test("computeReadiness locked cards → human_controlled", () => {
  ...
  let card = createCard("axiom", {
    one_sentence: "Test.",
    full_statement:
      "When testing this principle, the agent should apply specific judgment rules.",
