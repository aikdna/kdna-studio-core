...
test("shipped project-schema bytes are the runtime card-type authority", () => {
  ...
  assert.deepEqual(
    CARD_TYPES,
    PROJECT_SCHEMA.properties.cards.items.properties.type.enum,
  );
...
test("all 16 authoritative card types pass both JSON Schema and validateProject", () => {
  ...
  for (const type of CARD_TYPES) {
    ...
    assert.equal(
      ...
      true,
      `${type}: ${JSON.stringify(validateSchema.errors)}`,
    );
    assert.deepEqual(
...
test("an unsupported card type fails both JSON Schema and validateProject", () => {
  ...
  project.cards = [
    { id: "card_unknown", type: "shadow_card", status: "draft" },
  ];
...
test("a missing card-type enum fails closed", () => {
  ...
  assert.throws(
    () => parseProjectSchemaAuthority(bytes),
    /exactly 16 card types/,
  );
});
...
test("a truncated or expanded card-type enum fails closed", () => {
  ...
  assert.throws(
    () => parseProjectSchemaAuthority(truncated),
    /exactly 16 card types/,
  );
  assert.throws(
    () => parseProjectSchemaAuthority(expanded),
    /exactly 16 card types/,
  );
});
...
test("duplicate or non-string card-type declarations fail closed", () => {
  ...
  assert.throws(
    () => parseProjectSchemaAuthority(nonString),
    /non-empty strings/,
  );
});
