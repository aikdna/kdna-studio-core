...
test("the 3.x package root exposes only the admitted authoring primitives", () => {
  ...
  for (const name of [
    ...
    assert.equal(
      studio[name],
      undefined,
      `retired 2.x export leaked from 3.x package root: ${name}`,
    );
  }
});
...
test("source -> review -> confirm -> export preserves provenance and boundaries", () => {
  ...
  assert.deepEqual(axiom.applies_when, [
    "Reviewing the structure of a long-form article",
  ]);
  assert.deepEqual(axiom.does_not_apply_when, [
    "The request is limited to spelling or punctuation",
  ]);
  assert.equal(
    axiom.failure_risk,
    "A structural review may exceed the requested editing scope.",
  );
});

test("AI-derived content cannot claim a person or organization without matching confirmation", () => {
  const project = studio.authoring.createProject(
    "@example/represented-judgment",
  );
  const card = addJudgment(project, {
...
test("source identity is explicit and never defaults to AI or a human owner", () => {
  ...
  assert.throws(
    () => addJudgment(project, { sourceType: undefined }),
    /sourceType is required/,
  );
  assert.throws(
    () => addJudgment(project, { sourceType: "unknown" }),
