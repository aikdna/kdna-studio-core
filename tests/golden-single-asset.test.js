...
test("Golden synthetic authoring source preserves exact values through compiled JSON and CBOR payload", () => {
  ...
  const sourceSelfCheck = project.cards.find(
    (card) => card.type === "self_check",
  );
  ...
  assert.deepEqual(compiledPatterns.self_check, [
    sourceSelfCheck.fields.question,
  ]);
  ...
  assert.deepEqual(decodedPayload.reasoning.self_check, [
    sourceSelfCheck.fields.question,
  ]);
});
...
test("Golden compiled domain lints clean and runtime export passes every public Core validation gate", () => {
  ...
  const directory = fs.mkdtempSync(
    path.join(os.tmpdir(), "kdna-studio-golden-"),
  );
...
test("project validation rejects malformed or unsupported judgment_core declarations", () => {
  ...
  assert.ok(
    result.issues.includes(
      "judgment_core.value_order[1]: expected a non-empty string",
    ),
  );
  assert.ok(
    result.issues.includes("judgment_core.policy_table: unsupported field"),
  );
});
