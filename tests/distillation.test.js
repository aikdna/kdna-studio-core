...
test("createDistillationTarget — creates valid target", () => {
  ...
  assert.deepStrictEqual(target.exclude_areas, [
    "life habits",
    "food preference",
  ]);
...
test("createDistillationTarget — domainName required", () => {
  assert.throws(
    () => distillation.createDistillationTarget({}),
...
test("applyScopeGate — in-scope candidate passes", () => {
  ...
  const candidate = {
    ...
    full_statement:
      "In writing, always check argument structure first, then expression.",
...
test("applyScopeGate — exclude match fails scope", () => {
  ...
  const candidate = {
    ...
    full_statement:
      "Life habits and consumption choices include daily coffee ritual.",
    candidate_status: "proposed",
  };
...
test("applyScopeGate — suggests split domain", () => {
  ...
  const candidate = {
    ...
    full_statement:
      "Prefers communication style that is direct and clear, related to decision preference.",
    candidate_status: "proposed",
  };
...
test("checkSensitiveContent — detects English medical content", () => {
  const result = distillation.checkSensitiveContent(
    "Based on their medical condition and therapy history",
  );
...
test("checkSensitiveContent — detects Chinese medical content", () => {
  const result =
    distillation.checkSensitiveContent("根据其心理疾病诊断和药物治疗情况");
  assert.equal(result.flagged, true);
});

test("checkSensitiveContent — detects Chinese financial content", () => {
  const result =
    distillation.checkSensitiveContent("他的月收入达到五万元，工资水平较高");
  assert.equal(result.flagged, true);
});

test("checkSensitiveContent — detects Chinese political content", () => {
  const result =
    distillation.checkSensitiveContent("他因为政治立场不同而选择离开");
  assert.equal(result.flagged, true);
});

test("checkSensitiveContent — passes normal Chinese content", () => {
  const result = distillation.checkSensitiveContent(
    "用户偏好简洁有力的写作风格，强调论证结构优先于文字润色",
  );
...
test("SENSITIVE_KEYWORDS — all 6 domains present with bilingual entries", () => {
  ...
  for (const domain of domains) {
    assert.ok(
      kw[domain].length >= 4,
      `${domain} should have at least 4 keywords`,
    );
