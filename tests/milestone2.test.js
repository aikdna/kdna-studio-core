...
const {
  validateCard,
  validateAllCards,
} = require("../src/quality/validate-cards");
...
function makeLockedCard(type, fields = {}, id = null) {
  const f = { ...fields };
  if (type === "axiom") {
    if (!f.full_statement || f.full_statement.length < 20)
      f.full_statement =
        "A complete testable explanation of this judgment principle with sufficient detail for the agent to apply it correctly in real scenarios.";
    if (!f.why || f.why.length < 20)
      f.why =
        "Without this axiom the agent would make incorrect judgment calls resulting in poor outputs.";
  }
...
describe("Quality Gates", () => {
  ...
  test("draft_grade: no Human Lock remains compilable with a provenance warning", () => {
    ...
    assert.equal(
      r.blocking.some((b) => b.includes("nothing to compile")),
      false,
    );
  });
  ...
  test("human_controlled: 3+ locked axioms with boundaries ", () => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      attachRestatementToLock(
        ...
        createFeynmanRestatement(
          ax,
          `Simple explanation for axiom ${i}: when you see this situation, do this instead of that, but only if the conditions are right.`,
        ),
      );
  ...
  test("tested_grade: 5+ rated tests checks evals requirement", () => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      attachRestatementToLock(
        ...
        createFeynmanRestatement(
          ax,
          `Plain explanation for axiom ${i}: when this happens, the agent should do this specific thing instead of that, unless these boundary conditions apply.`,
        ),
      );
      project.cards.push(ax);
    }
    for (let i = 0; i < 3; i++) {
      const sc = makeLockedCard("self_check", {
        question: `Does the response satisfy condition ${i}?`,
      });
    ...
    for (let i = 0; i < 6; i++) {
      ...
      recordHumanRating(
        tc,
        i < 4 ? "with_kdna_better" : "no_difference",
        "tester",
      );
  ...
  test("publishable_grade: 10+ evals, 3+ axioms, 5+ self-checks, all Feynman", () => {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      attachRestatementToLock(
        ...
        createFeynmanRestatement(
          ax,
          `Simple: when situation X happens, the agent should do Y instead of Z. But this only applies when condition A is true — if not, skip it.`,
        ),
      );
      project.cards.push(ax);
    }
    for (let i = 0; i < 5; i++) {
      project.cards.push(
        makeLockedCard("self_check", {
          question: `Does the output pass criterion ${i}?`,
        }),
      );
    }
    for (let i = 0; i < 12; i++) {
    ...
    assert.ok(
      r.grade === "publishable_grade" ||
        r.grade === "tested_grade" ||
        r.grade === "human_controlled",
    );
...
describe("Card Validation", () => {
  test("flags slogan-like axiom", () => {
    let card = createCard("axiom", {
      one_sentence: "Trust is important for teams.",
      full_statement: "Trust matters.",
    });
    const issues = validateCard(card);
    assert.ok(
      issues.some((i) => i.type === "slogan" || i.type === "too_short"),
    );
  });
  ...
  test("validateAllCards aggregates per-card", () => {
    ...
    project.cards = [
      createCard("axiom", {
        one_sentence: "Trust is key.",
        full_statement: "Trust matters in everything.",
      }),
...
describe("Full Compile", () => {
  ...
  test("produces Reasoning from axiom implications", () => {
    ...
    project.cards = [
      makeLockedCard("axiom", {
        one_sentence: "Price objections are certainty deficits.",
        full_statement:
          "When a buyer says too expensive, first diagnose which type of uncertainty is blocking them.",
        why: "Without this axiom, agents default to offering discounts instead of diagnosing.",
        applies_when: ["price objection"],
        does_not_apply_when: ["explicit discount request"],
    ...
    assert.equal(
      reasoning.reasoning_chains[0].so_what,
      "Without this axiom, agents default to offering discounts instead of diagnosing.",
    );
  });
  ...
  test("Fix 2: Reasoning preserves source identity when source.reasoning is provided", () => {
    ...
    project.cards = [
      makeLockedCard(
        "axiom",
        {
          ...
          full_statement:
            "A complete testable explanation of this judgment principle with sufficient detail for the agent to apply it correctly.",
      ...
      makeLockedCard(
        "reasoning",
        {
          ...
          chain: [
            "Diagnose uncertainty type",
            "Address the specific gap",
            "Reframe value",
          ],
          concrete_action:
            "Ask which dimension of certainty is missing before responding to price.",
        },
        "RC-001",
      ),
    ];
    ...
    assert.equal(
      chain.so_what,
      "Ask which dimension of certainty is missing before responding to price.",
      "concrete_action maps to so_what",
    );
    assert.equal(
      chain.one_sentence,
      "Source-authored principle statement.",
      "one_sentence preserved",
    );
    assert.deepEqual(
      chain.logic,
      [
      ...
      "chain array maps to logic",
    );
    assert.equal(
      chain.principle,
      "Price objections are certainty deficits.",
      "principle preserved as additional field",
    );
    assert.equal(
      chain.concrete_action,
      "Ask which dimension of certainty is missing before responding to price.",
      "concrete_action preserved as additional field",
    );
  });
  ...
  test("Fix 2: Reasoning falls back to axiom synthesis when no source provided", () => {
    ...
    project.cards = [
      makeLockedCard(
        "axiom",
        {
          ...
          full_statement:
            "A complete testable explanation of this judgment principle with sufficient detail.",
    ...
    assert.equal(reasoning.reasoning_chains.length, 1);
    assert.equal(
      reasoning.reasoning_chains[0].id,
      "chain_AX-001",
      "axiom synthesis path unchanged",
    );
    assert.equal(
      reasoning.reasoning_chains[0].so_what,
      "Without this axiom, agents would default to incorrect behavior in this domain.",
    );
  });
  ...
  test("Fix 3: Evolution prepends source-authored stages when source.evolution is provided", () => {
      ...
      source: {
        evolution: {
          stages: [
            ...
            {
              id: "stage_experimental",
              name: "Experimental",
              level: 1,
              description: "Tested on real tasks.",
            },
            {
              id: "stage_canonical",
              name: "Canonical",
    ...
    assert.equal(
      draftStage.level,
      0,
      "level 0 preserved (not coerced to empty string)",
    );
    assert.equal(
      auditStages.length,
      1,
      "audit-log stage still emitted (AX-001 lock)",
    );
  });
  ...
  test("Fix 3: Evolution preserves locked evolution_stage cards without source.evolution", () => {
    ...
    const result = compileDomain(project);
    const evo = JSON.parse(result.files["KDNA_Evolution.json"]);
    const sourceStage = evo.stages.find(
      (s) => s.id === "stage_source_imported",
    );
    assert.ok(sourceStage, "locked evolution_stage card is preserved");
    assert.equal(
      sourceStage.source_authored,
      true,
      "locked evolution_stage remains source-authored",
    );
    assert.equal(sourceStage.name, "Source Imported");
    assert.equal(sourceStage.level, 2, "numeric level preserved");
    assert.equal(
      sourceStage.description,
      "A source-authored stage imported as a Studio card.",
    );
  });
  ...
  test("Fix 3: Evolution falls back to audit-log-only when no source provided", () => {
    ...
    assert.equal(
      sourceStages.length,
      0,
      "no source stages when source not provided",
    );
  });
  ...
  test("Fix 1: term cards compile into terminology.standard_terms (was dropped)", () => {
    ...
    project.cards = [
      ...
      makeLockedCard(
        "term",
        {
          term: "failure mode",
          definition:
            "A predictable, repeating class of agent error in a specific situation.",
        },
        "term_001",
      ),
    ];
    ...
    assert.equal(
      patterns.terminology.standard_terms.length,
      1,
      "1 standard_term compiled",
    );
    assert.equal(patterns.terminology.standard_terms[0].term, "failure mode");
    assert.equal(
      patterns.terminology.standard_terms[0].definition,
      "A predictable, repeating class of agent error in a specific situation.",
    );
  });
  ...
  test("Fix 1: banned_term cards compile into terminology.banned_terms (was dropped)", () => {
    ...
    project.cards = [
      ...
      makeLockedCard(
        "banned_term",
        {
          ...
          replace_with:
            "Identify the constraint, then choose the minimum action that resolves it.",
        },
        "ban_001",
      ),
    ];
    ...
    assert.ok(patterns.terminology, "patterns.terminology section exists");
    assert.equal(
      patterns.terminology.banned_terms.length,
      1,
      "1 banned_term compiled",
    );
    assert.equal(patterns.terminology.banned_terms[0].term, "just do it");
    assert.equal(
      patterns.terminology.banned_terms[0].why,
      "Bypasses judgment; produces action without diagnosis.",
    );
    assert.equal(
      patterns.terminology.banned_terms[0].replace_with,
      "Identify the constraint, then choose the minimum action that resolves it.",
    );
  });
});
  ...
  test("generates README with 4 questions", () => {
    ...
    project.cards = [
      makeLockedCard("axiom", {
        ...
        full_statement:
          "When a team fails to execute, first check whether a real decision was ever made.",
        ...
        failure_risk:
          "May cause over-scrutiny of decision quality when issue is resources.",
      }),
      makeLockedCard("misunderstanding", {
        wrong: "If the team is not executing, they lack motivation.",
        correct:
          "If the team is not executing, first check whether a decision with owner+deadline+criteria was made.",
        key_distinction:
          "Motivation gaps produce gradual decline. Decision voids produce sudden stalls.",
      }),
      makeLockedCard("self_check", {
        question:
          "Did I verify that a concrete decision exists before diagnosing the team?",
      }),
    ];
    project.tests = [];
    for (let i = 0; i < 6; i++) {
    ...
    const readme = generateReadme(project, {
      description:
        "Leadership decision-making judgment — diagnose whether execution failures are really decision voids.",
    ...
    assert.ok(
      readme.includes(
        "authored axioms with applies_when / does_not_apply_when / failure_risk",
      ),
    );
  ...
  test("unreviewed cards are documented without fabricated Human Lock provenance", () => {
    ...
    project.cards = [
      createCard("axiom", {
        ...
        full_statement:
          "A structurally valid KDNA asset can be authored without claiming a human review event.",
    ...
    assert.match(
      readme,
      /1 non-deprecated judgment cards authored with KDNA Studio/,
    );
...
describe("Provenance Integration", () => {
  test("buildProvenance captures compile metadata", () => {
    const project = createProject("test", "domain", {
      author: { name: "Author", id: "auth_001" },
    });
    project.cards = [
      makeLockedCard("axiom", {
        one_sentence: "T.",
