...
function compileFixture(access = "public") {
  ...
  project.cards.push(
    createCard(
      ...
      {
        one_sentence:
          "A report type and its compatibility coordinate are separate.",
        full_statement:
        ...
        failure_risk:
          "Consumers may route the report using an ambiguous identifier.",
      },
      "ax_report_contract",
    ),
  );
...
test("report schemas reject a wrong responsibility type or compatibility coordinate", () => {
  ...
  assert.equal(
    validate({ ...report, type: "kdna.studio.unrelated-report" }),
    false,
  );
...
test("protocol contract keeps responsibility names separate from coordinates", () => {
  ...
  assert.equal(
    protocolContract.RUNTIME_ENTRY_SET_DIGEST_PROFILE_VERSION,
    "0.1.0",
  );
});
