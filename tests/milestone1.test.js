...
} = require("../src/quality/contradiction");
...
describe("Feynman Restatement", () => {
  test("createFeynmanRestatement scores a good restatement", () => {
    let card = makeLockedCard("axiom", {
      one_sentence:
        "Price objections are certainty deficits, not price problems.",
    });
    const text =
      "When a customer says the price is too high, the agent should not immediately offer a discount. Instead, figure out which type of uncertainty is blocking them — value, risk, responsibility, or process. For example, if a client hesitates, ask what they are worried about rather than lowering the price.";
  ...
  test("createFeynmanRestatement detects just-repeating", () => {
    let card = makeLockedCard("axiom", {
      one_sentence:
        "Clarity is the only thing a writer needs to worry about when creating content.",
    });
    const text =
      "Clarity is the only thing a writer needs to worry about when creating content for readers.";
    const fr = createFeynmanRestatement(card, text);
  ...
  test("attachRestatementToLock links restatement to card", () => {
    ...
    const fr = createFeynmanRestatement(
      card,
      "A simple explanation with an example and a boundary: this only works when there is clear evidence, not when the situation is ambiguous.",
    );
...
describe("Contradiction Check", () => {
  ...
  test("detects over-generalized language", () => {
    let card = makeLockedCard("axiom", {
      ...
      full_statement:
        "All user input must always be validated before processing.",
...
describe("Evidence", () => {
  test("createEvidenceEntry generates ID and hash", () => {
    const ev = createEvidenceEntry(
      "text",
      "Test Evidence",
      "This is some test content for judgment extraction.",
    );
  ...
  test("extractSpan creates annotation", () => {
    const ev = createEvidenceEntry(
      "text",
      "Test",
      "The expert said that price objections are really about uncertainty, not actual price concerns.",
    );
...
describe("Test Lab", () => {
  ...
  test("recordHumanRating sets result", () => {
    ...
    recordHumanRating(
      ...
      "with_kdna_better",
      "tester",
      "KDNA clearly improved the diagnosis.",
    );
  ...
  test("generateTestSummary calculates stats", () => {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      recordHumanRating(
        tc,
        i < 3 ? "with_kdna_better" : "no_difference",
        "tester",
      );
...
describe("Full Authoring Workflow", () => {
  test("end-to-end: evidence → card → lock → Feynman → contradiction → compile", () => {
    ...
    const ev1 = createEvidenceEntry(
      "text",
      "Expert Interview",
      'In 15 years of leadership coaching, I have noticed that most "execution failures" are actually decisions that were never properly made in the first place.',
      "manual",
    );
    addEvidence(project, ev1);

    // Stage 2: Create judgment cards
    const ax1 = createCard(
      "axiom",
      {
        one_sentence:
          "Execution failure is often decision failure in disguise.",
        full_statement:
          'When a team fails to execute, first check whether a real decision (with named owner, deadline, and criteria) was ever made. Most "execution problems" are decision voids.',
        ...
        does_not_apply_when: [
          "Clear decision exists with owner",
          "External blocker (vendor, regulation)",
        ],
        failure_risk:
          "May cause managers to over-scrutinize decision quality when the real issue is resource availability.",
      },
      "ax_001",
    );
    project.cards.push(ax1);

    const ms1 = createCard(
      "misunderstanding",
      {
        wrong: "If the team is not executing, they lack motivation or skills.",
        correct:
          "If the team is not executing, first check whether a real decision was ever made — with owner, deadline, and criteria.",
        key_distinction:
          "Motivation gaps produce gradual decline. Decision voids produce sudden stalls. The pattern is different.",
      },
      "ms_001",
    );
    project.cards.push(ms1);

    const sc1 = createCard(
      "self_check",
      {
        question:
          "Before suggesting that the team lacks motivation, did I verify that a concrete decision with owner+deadline+criteria exists?",
      },
      "sc_001",
    );
    ...
    for (let card of [ax1, ms1, sc1]) {
      ...
      card = lockCard(card, {
        by: "expert_001",
        statement:
          "This judgment represents my 15 years of leadership coaching experience.",
        checked: {
          applies_when: true,
          does_not_apply_when: true,
          failure_risk: true,
    ...
    for (const card of project.cards) {
      ...
      assert.ok(
        ...
        `Expected good or acceptable, got ${quality} for ${card.id}`,
      );
    }
