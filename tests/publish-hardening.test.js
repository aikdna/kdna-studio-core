...
const {
  ...
  expectedE404,
} = require("../scripts/registry-policy");
const {
...
test("authoritative Git selects a Git-compatible null device on every platform", () => {
  ...
  assert.equal(
    authoritativeGitEnvironment({}).GIT_REPLACE_REF_BASE,
    "refs/replace/",
  );
});
...
function releaseTarball() {
  return tarGzip([
    { name: "package/package.json", content: '{"name":"studio-release"}\n' },
  ]);
}
...
test("publish workflow is release-only, serialized, pinned, and publishes one verified tarball", () => {
  ...
  assert.match(
    workflow,
    new RegExp(
      `node --version \\| grep -Fx .*${["22", "23", "1"].join("\\.")}`,
    ),
  );
  ...
  assert.match(
    workflow,
    /run-trusted-npm\.js run release:generate-evidence --/,
  );
  ...
  assert.equal(
    scripts["release:generate-evidence"],
    "node scripts/generate-release-evidence.js",
  );
  assert.equal(
    scripts["release:publish-verified"],
...
test("release Git authority ignores hostile environment injection and rejects replacement refs", (t) => {
  ...
  fs.writeFileSync(
    path.join(release.repository, "marker.txt"),
...
test("release evidence entry rejects hidden worktree changes and packs exact commit blobs", (t) => {
  ...
  fs.writeFileSync(
    path.join(release.repository, "marker.txt"),
    "hidden-release-evidence-change\n",
  );
  git(release.repository, ["update-index", "--assume-unchanged", "marker.txt"]);
  assert.throws(
    () =>
  ...
  const files = parseTarFiles(fs.readFileSync(artifact)).map(
    (entry) => entry.path,
  );
...
test("release evidence requires byte-identical isolated npm packs", () => {
  ...
  assert.doesNotThrow(() =>
    assertReproduciblePackBytes(bytes, Buffer.from(bytes)),
  );
  assert.throws(
    () =>
      assertReproduciblePackBytes(
        bytes,
        Buffer.from("different release bytes"),
      ),
    /not byte-identical/,
  );
...
test("every audited npm child receives the controlled execution environment", () => {
  const consumers = new Map([
    ...
    [
      "scripts/generate-release-evidence.js",
      ["env: npmInvocation.environment", 1],
    ],
    [
      "scripts/check-current-protocol-names.js",
      ["env: invocation.environment", 1],
    ],
    [
      "scripts/publish-verified-artifact.js",
      ["env: npmInvocation.environment", 1],
    ],
    [
      "scripts/verify-runtime-candidate-sources.js",
    ...
    [
      "scripts/runtime-candidate-binding.js",
      ["env: invocation.environment", 1],
    ],
  ]);
...
test("current binding rejects stale evidence before registry lookup", () => {
  ...
  assert.equal(
    validateCurrentBinding({ evidence, ...releaseInput() }),
    evidence,
  );
  let calls = 0;
  assert.throws(() =>
...
test("pack evidence independently parses a real npm tgz and rejects changed bytes", (t) => {
  const temp = fs.mkdtempSync(
    path.join(os.tmpdir(), "studio-release-pack-test-"),
  );
  ...
  assert.throws(
    () => validateArtifact(evidence, Buffer.from("changed")),
    /size|integrity|shasum/,
  );
  assert.throws(
    () =>
      validateArtifact(
        {
          ...
          artifact: {
            ...evidence.artifact,
            unpacked_size: evidence.artifact.unpacked_size + 1,
          },
        },
        bytes,
      ),
  ...
  assert.throws(
    () =>
      validateArtifact(
        {
          ...evidence,
          artifact: { ...evidence.artifact, filename: "../release.tgz" },
        },
        bytes,
      ),
    /filename mismatch/,
...
test("independent tar parser rejects non-gzip, truncation, checksum damage, bad endings, duplicates, and unsafe paths", () => {
  ...
  const cases = [
    ...
    [
      "symbolic link",
      tarGzip([{ name: "package/link", type: "2", content: "" }]),
      /unsupported/,
    ],
  ];
...
test("registry policy publishes only on the exact npm 11 E404 and skips only identical bytes", () => {
  ...
  for (const result of [
    ...
    {
      ...present,
      stdout: present.stdout.replace(evidence.artifact.shasum, "b".repeat(40)),
    },
  ]) {
