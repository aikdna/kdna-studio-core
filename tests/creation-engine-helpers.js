...
function purposeFor(mode) {
  return {
    ...
    objective:
      "Prioritize reversible incident actions before speculative repair.",
    scope: "service incident triage",
    non_goals: ["Never reveal credentials or private source content."],
    loading_condition:
      "Load when choosing the next action in a service incident.",
    represented_subject: MODE_SUBJECTS[mode],
    highest_question:
      "Which action reduces irreversible harm while preserving diagnosis?",
...
function candidateFor({
  ...
  return {
    ...
    rationale:
      "The bounded choice preserves evidence, reversibility, and declared authority.",
    ...
    counterexample_search: {
      ...
      method:
        "Review a reversible case and an urgent irreversible counterexample.",
      result: "found",
      uncertainty:
        "Scenarios outside the declared incident scope were not evaluated.",
    },
    confidence: {
      status: "high",
      score: 0.95,
...
function createPromotedWorkspace(mode = "agent-authored", options = {}) {
  ...
  const sourceRequired =
    !["agent-authored", "mixed-authorship"].includes(mode) ||
    options.withMaterial;
  if (sourceRequired) {
    workspace = creationEngine.ingestMaterial(workspace, {
      ...
      content:
        "Preserve evidence and reversibility while the failure mode remains uncertain.",
...
function addModeConfirmation(workspace) {
  ...
  if (workspace.state.mode === "organization-confirmed") {
    return creationEngine.recordConfirmation(workspace, {
      ...
      scope: "model",
      statement:
        "I am authorized to confirm this semantic revision for the organization.",
    });
  }
  return workspace;
}
...
function freezeSemanticCases(workspace, definitions, options = {}) {
  let next = workspace;
...
function acceptWorkspace(workspace, options = {}) {
  ...
  for (const relation of next.judgmentModel.relations.filter(
    ...
    definitions.push({
      ...
      kind: "conflict",
      input: "A task activates the declared relation between two judgments.",
      expected:
