...
function copyAuthorityRoot(t) {
  const root = fs.mkdtempSync(
    path.join(os.tmpdir(), "studio-core-candidate-hardening-"),
  );
  ...
  const binding = JSON.parse(
    fs.readFileSync(path.join(ROOT, BINDING_PATH), "utf8"),
  );
...
function createSourceRepository(
  t,
  packageJson = { name: CORE, version: "0.21.0" },
) {
  const repository = fs.mkdtempSync(
    path.join(fs.realpathSync(os.tmpdir()), "studio-core-source-repo-"),
  );
...
test("static graph rejects the reproduced npm alias even when locked.name is omitted", (t) => {
  ...
  mutateJson(root, "package-lock.json", (lock) => {
    lock.packages[""].dependencies["shadow-core"] =
...
test("candidate commits are bound to canonical CI pins and evidence", (t) => {
  ...
  assert.throws(
    () => verifyCandidateBinding(root),
    /does not match the CI pin/,
  );

  const evidenceRoot = copyAuthorityRoot(t);
  mutateJson(
    evidenceRoot,
    CANDIDATE_AUTHORITIES[0].evidencePath,
    (evidence) => {
      evidence.git_head = "b".repeat(40);
    },
  );
  assert.throws(
    () => verifyCandidateBinding(evidenceRoot),
  ...
  mutateJson(policyRoot, CANDIDATE_AUTHORITIES[0].evidencePath, (evidence) => {
    evidence.pack.source_equivalence.excluded_non_install_metadata.push(
      "tar_mode",
    );
  });
  assert.throws(
    () => verifyCandidateBinding(policyRoot),
    /candidate pack evidence mismatch/,
  );

  const evidence = JSON.parse(
    fs.readFileSync(
      path.join(ROOT, CANDIDATE_AUTHORITIES[0].evidencePath),
      "utf8",
    ),
  );
  assert.equal(evidence.pack.reproducible_runs, 2);
  assert.deepEqual(
    evidence.pack.source_equivalence,
    STRICT_PACKAGE_INSTALL_EQUIVALENCE,
  );
});
...
test("candidate CI pins immutable actions, exact Node runtimes, and verified npm bytes", () => {
  const workflow = fs.readFileSync(
    path.join(ROOT, CANDIDATE_WORKFLOW_PATH),
    "utf8",
  );
  ...
  assert.equal(
    codeql.match(
      /github\/codeql-action\/(?:init|autobuild|analyze)@99df26d4f13ea111d4ec1a7dddef6063f76b97e9/g,
    )?.length,
    3,
  );
  ...
  assert.match(
    workflow,
    /run-trusted-npm\.js run verify:candidate-sources\s*$/m,
  );
  ...
  assert.doesNotMatch(
    workflow,
    /KDNA_CORE_CANDIDATE_SOURCE:\s*\$\{\{\s*github\.workspace/,
  );
...
test("candidate source equivalence rejects malformed excluded tar metadata", async (t) => {
  const checked = syntheticTarGzip([
    { name: "package/index.js", content: "abc" },
  ]);
...
test("trusted npm wrapper binds execution to the repository root", (t) => {
  const foreign = fs.mkdtempSync(
    path.join(fs.realpathSync(os.tmpdir()), "studio-core-foreign-cwd-"),
  );
...
test("trusted npm wrapper forwards the controlled lifecycle environment", (t) => {
  ...
  const hostileBin = fs.mkdtempSync(
    path.join(
      ...
      "studio-core-wrapper-hostile-path-",
    ),
  );
...
test("every candidate authority rejects symlinks and hard links", async (t) => {
  const files = [
    ...
    [
      JSON.parse(fs.readFileSync(path.join(ROOT, BINDING_PATH), "utf8"))
        .packages[0].artifact,
      "candidate artifact",
    ],
  ];
...
test("installed graph rejects aliases, descendants, vendored identities, symlinks, and hard links", async (t) => {
  const canonical = (root) => writeInstalledPackage(root, CORE, CORE, "0.21.0");
  ...
  for (const [name, mutation, pattern] of [
    [
      "alias",
      (root) => writeInstalledPackage(root, "shadow-core", CORE, "0.18.0"),
      /canonical top-level/,
    ],
    [
      "descendant",
      (root) =>
        writeInstalledPackage(
          root,
          "foreign/dist/node_modules/shadow-core",
          CORE,
          "0.21.0",
        ),
      /canonical top-level/,
    ...
    [
      ...
      (root) =>
        writeInstalledPackage(
          root,
          "foreign/vendor/deep/shadow-core",
          CORE,
          "0.21.0",
        ),
      /canonical top-level/,
    ...
    [
      ...
      (root) =>
        writeInstalledPackage(
          root,
          ".bin/node_modules/shadow-core",
          CORE,
          "0.21.0",
        ),
      /\.bin must not contain directories/,
    ],
  ]) {
    await t.test(name, (t) => {
      const root = copyAuthorityRoot(t);
  ...
  await t.test("package symlink", (t) => {
    ...
    assert.throws(
      ...
      /package graph contains a symlink/,
    );
  });
  await t.test("manifest hard link", (t) => {
    ...
    fs.linkSync(
      ...
      path.join(root, "installed-core-hardlink.json"),
    );
    assert.throws(
      () => verifyInstalledAikdnaGraph(root),
      /exactly one hard link/,
...
test("registry lookup uses only the integrity-anchored npm release and canonical registries", (t) => {
  const fakeRoot = fs.mkdtempSync(
    path.join(fs.realpathSync(os.tmpdir()), "studio-core-fake-npm-"),
  );
  ...
  fs.writeFileSync(
    fakeExecPath,
    "require('node:fs').writeFileSync(process.env.FAKE_NPM_MARKER, 'ran');\n",
  );
  fs.writeFileSync(
  ...
  const metadata = strictRegistryLookup(CORE, "0.21.0", {
    root: ROOT,
  ...
  assert.ok(
    invocation.args.includes("--@aikdna:registry=https://registry.npmjs.org/"),
  );
  ...
  fs.writeFileSync(
    ...
    fs.readFileSync(
      path.join(
        ROOT,
        BINDING_PATH.replace("binding.json", "kdna-core-0.21.0.tgz"),
      ),
    ),
  );
  assert.throws(
...
test("trusted npm cache paths canonicalize parent aliases and replace invalid bytes", async (t) => {
  const temporary = fs.mkdtempSync(
    path.join(fs.realpathSync(os.tmpdir()), "studio-core-npm-path-"),
  );
  t.after(() => fs.rmSync(temporary, { recursive: true, force: true }));
  ...
  assert.throws(
    () => verifyTrustedNpmTarball(symlinkTar),
    /regular non-symlink/,
  );
});
...
test("candidate source authority rejects replace refs and archives the original commit", (t) => {
  ...
  const isolated = fs.mkdtempSync(
    path.join(fs.realpathSync(os.tmpdir()), "studio-core-no-replace-"),
  );
  ...
  git(fixture.repository, [
    "update-ref",
    `refs/alternate-replacements/${fixture.commit}`,
    replacement,
  ]);
  ...
  assert.equal(
    rewrittenArchive.status,
    0,
    rewrittenArchive.stderr?.toString("utf8"),
  );
  assert.ok(
    rewrittenArchive.stdout.includes(Buffer.from("replacement-content")),
  );

  const sanitized = fs.mkdtempSync(
    path.join(fs.realpathSync(os.tmpdir()), "studio-core-sanitized-git-"),
  );
...
test("commit-object materialization ignores private export attributes", (t) => {
  ...
  fs.writeFileSync(
    path.join(fixture.source, "substituted.txt"),
    substitutedSource,
  );
  ...
  assert.equal(oldArchive.status, 0, oldArchive.stderr?.toString("utf8"));
  assert.equal(
    oldArchive.stdout.includes(Buffer.from("module.exports = true")),
    false,
  );
  assert.equal(
    oldArchive.stdout.includes(Buffer.from(substitutedSource)),
    false,
  );
  ...
  assert.equal(
    fs.readFileSync(path.join(materialized, "substituted.txt"), "utf8"),
    substitutedSource,
  );
});
...
test("commit-object materialization accepts executable blobs and rejects special modes", async (t) => {
  await t.test("executable blob", (t) => {
    ...
    assert.equal(
      fs.statSync(path.join(materialized, "index.js")).mode & 0o777,
      0o755,
    );
  });
...
test("formal candidate evidence keeps two byte-identical trusted npm packs", (t) => {
  const marker = path.join(
    fs.realpathSync(os.tmpdir()),
    `studio-core-lifecycle-${process.pid}-${Date.now()}`,
  );
  ...
  const packageJson = {
    ...
    scripts: {
      prepack:
        "node -e \"require('node:fs').writeFileSync(process.env.FAKE_NPM_MARKER,'ran')\"",
    },
  };
  const fixture = createSourceRepository(t, packageJson);
  const isolated = fs.mkdtempSync(
    path.join(fs.realpathSync(os.tmpdir()), "studio-core-commit-tree-"),
  );
  t.after(() => fs.rmSync(isolated, { recursive: true, force: true }));
  materializeCommitPackage(
    fixture.repository,
  ...
  const fakeRoot = fs.mkdtempSync(
    path.join(fs.realpathSync(os.tmpdir()), "studio-core-copy-npm-"),
  );
  t.after(() => fs.rmSync(fakeRoot, { recursive: true, force: true }));
  ...
  fs.writeFileSync(
    ...
    [
      ...
      "const filename = 'aikdna-kdna-core-0.21.0.tgz';",
  ...
  process.env.FAKE_COPY_ARTIFACT = path.join(
    ROOT,
    "fixtures/runtime-candidates/kdna-core-0.21.0.tgz",
  );
  t.after(() => {
  ...
  assert.notDeepEqual(
    firstBytes,
    fs.readFileSync(
      path.join(ROOT, "fixtures/runtime-candidates/kdna-core-0.21.0.tgz"),
    ),
  );
});
...
test("published package contains no candidate tar, binding, or evidence files", (t) => {
  const output = fs.mkdtempSync(
    path.join(os.tmpdir(), "studio-core-public-pack-"),
  );
  ...
  assert.deepEqual(
    report.files.filter((file) =>
      /runtime-candidates|\.tgz$|evidence\.json$/.test(file.path),
    ),
    [],
  );
});
