...
function createSourceRepository(t, packageJson = { name: CORE, version: '0.21.0' }) {
...
test('installed graph rejects aliases, descendants, vendored identities, symlinks, and hard links', async (t) => {
  ...
  for (const [name, mutation, pattern] of [
    ...
    ['descendant', (root) => writeInstalledPackage(root, 'foreign/dist/node_modules/shadow-core', CORE, '0.21.0'), /canonical top-level/],
    ['vendored', (root) => writeInstalledPackage(root, 'foreign/vendor/deep/shadow-core', CORE, '0.21.0'), /canonical top-level/],
    ['bin descendant', (root) => writeInstalledPackage(root, '.bin/node_modules/shadow-core', CORE, '0.21.0'), /\.bin must not contain directories/],
  ]) {
    await t.test(name, (t) => {
      const root = copyAuthorityRoot(t);
      canonical(root);
...
test('registry lookup uses only the integrity-anchored npm release and canonical registries', (t) => {
  ...
  const metadata = strictRegistryLookup(CORE, '0.21.0', {
    ...
    runner: (command, args, options) => {
      ...
      return {
        status: 0,
        signal: null,
        stdout: JSON.stringify({ name: CORE, version: '0.21.0', 'dist.integrity': integrity }),
        stderr: '',
      };
    },
  });
  ...
  fs.writeFileSync(falseTar, fs.readFileSync(path.join(ROOT, BINDING_PATH.replace('binding.json', 'kdna-core-0.21.0.tgz'))));
  assert.throws(
...
test('formal candidate evidence keeps two byte-identical trusted npm packs', (t) => {
  ...
  assert.notDeepEqual(
    firstBytes,
    fs.readFileSync(path.join(ROOT, 'fixtures/runtime-candidates/kdna-core-0.21.0.tgz')),
  );
});
