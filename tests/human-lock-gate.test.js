...
test("checkHumanLockGate: blocks when axiom is not approved for Studio export", () => {
  ...
  assert.ok(
    gate.issues.some((i) =>
      i.reason.includes("not approved for Studio export"),
    ),
  );
});
...
test("checkHumanLockGate: passes when axiom is properly locked", () => {
  ...
  let card = createCard("axiom", {
    one_sentence: "Test",
    full_statement:
      "A complete testable explanation of this judgment principle for the agent.",
    why: "Without this the agent would make wrong judgment calls.",
  });
...
test("exportProject: allows unreviewed cards without claiming Human Lock passed", () => {
  ...
  assert.throws(
    () => exportProject(p, { requireHumanLock: true }),
    /Human Lock Gate blocked export/,
  );
});
...
test("exportProject: succeeds when properly locked", () => {
  ...
  let card = createCard("axiom", {
    one_sentence: "Test",
    full_statement: "A complete testable explanation for the agent to apply.",
    why: "Without this axiom the agent would produce wrong results.",
  });
  card = transitionCard(card, "revised", { by: "tester" });
  card = lockCard(card, {
    by: "tester",
...
test("exportProject: force override allows blocked export", () => {
  ...
  const json = exportProject(p, {
    ...
    force: true,
    forceReason: "Critical security fix",
  });
  const parsed = JSON.parse(json);
...
test("exportProject: reviewed-only mode blocks a partial Human Lock", () => {
  ...
  let ax = createCard("axiom", {
    one_sentence: "Test 1",
    full_statement: "A complete testable explanation for agent use.",
    why: "Prevents incorrect agent judgment calls.",
  });
...
test("cardJudgmentFingerprint: different content produces different hash", () => {
  ...
  assert.notEqual(
    cardJudgmentFingerprint(card1),
    cardJudgmentFingerprint(card2),
  );
});
...
test("cardJudgmentFingerprint: covers every authored field, including nested protocol extensions", () => {
  ...
  assert.notEqual(
    cardJudgmentFingerprint(card1),
    cardJudgmentFingerprint(changedReference),
  );
  assert.notEqual(
    cardJudgmentFingerprint(card1),
    cardJudgmentFingerprint(changedRelation),
  );
});
...
test("cardJudgmentFingerprint: canonicalizes nested object keys without changing array order", () => {
  ...
  assert.equal(
    cardJudgmentFingerprint(card1),
    cardJudgmentFingerprint(sameContent),
  );
  assert.notEqual(
    cardJudgmentFingerprint(card1),
    cardJudgmentFingerprint(changedOrder),
  );
});
...
test("checkHumanLockGate: detects judgment content changed after lock", () => {
  const p = createProject("test");
  let card = createCard("axiom", {
    one_sentence: "Original",
    full_statement: "A complete testable explanation as originally designed.",
    why: "Without this the agent would produce incorrect results.",
  });
  card = transitionCard(card, "revised", { by: "tester" });
  card = lockCard(card, {
    by: "tester",
  ...
  assert.ok(
    gate.issues.some((i) => i.reason.includes("judgment fields changed")),
