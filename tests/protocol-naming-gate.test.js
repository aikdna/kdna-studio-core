...
test("naming gate rejects content, tag templates, identifiers, and filenames", (t) => {
  ...
  assert.ok(
    findings.some((finding) => finding.rule === "generation-style label"),
  );
  assert.ok(
    findings.some(
      (finding) => finding.rule === "generation-style tag template",
    ),
  );
  assert.ok(
    findings.some(
      (finding) =>
        finding.rule === "generation-style implementation identifier",
    ),
  );
  assert.ok(
    findings.some((finding) =>
      finding.file.includes(`report-${generation}.js`),
    ),
  );
});
