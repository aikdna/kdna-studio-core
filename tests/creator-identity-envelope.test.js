...
test("decryptPrivateKey: rejects non-object envelopes", () => {
  for (const bad of [
    ...
    assert.throws(
      () => decryptPrivateKey(bad, "pw"),
      /Invalid key envelope|not valid JSON/,
    );
  }
});
...
test("decryptPrivateKey: unknown KDF names are rejected before any derivation", () => {
  for (const kdf of [
    ...
    assert.throws(
      () => decryptPrivateKey(envelope, "pw"),
      /Unsupported key envelope KDF/,
    );
  }
});
...
test("decryptPrivateKey: rejects malformed or wrongly sized salt", () => {
  ...
  assert.throws(
    () => decryptPrivateKey(wrongLength, "pw"),
  ...
  assert.throws(
    () => decryptPrivateKey(notBase64, "pw"),
    /salt is not well-formed base64/,
  );

  const notString = makeEnvelope();
  notString.salt = 12345;
  assert.throws(
    () => decryptPrivateKey(notString, "pw"),
    /salt must be a non-empty base64 string/,
  );

  const unpadded = makeEnvelope();
  unpadded.salt = "Zg";
  assert.throws(
    () => decryptPrivateKey(unpadded, "pw"),
    /salt is not well-formed base64/,
  );
});
...
test("decryptPrivateKey: rejects non-canonical base64 that the lenient decoder would mangle", () => {
  ...
  assert.throws(
    () => decryptPrivateKey(envelope, "pw"),
    /tag is not (well-formed|canonical) base64/,
  );
});
...
test("decryptPrivateKey: rejects oversized ciphertext before the KDF runs", () => {
  ...
  assert.throws(
    () => decryptPrivateKey(envelope, "pw"),
    /ciphertext must decode to between 1 and 65536 bytes/,
  );
});
...
test("decryptPrivateKey: rejects empty and malformed ciphertext", () => {
  ...
  assert.throws(
    () => decryptPrivateKey(malformed, "pw"),
    /ciphertext is not well-formed base64/,
  );
});
