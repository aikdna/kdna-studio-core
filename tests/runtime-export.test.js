...
function createRuntimeProject(access = "public") {
  ...
  project.cards.push(
    makeLockedCard(
      "axiom",
      {
        ...
        full_statement:
          "Studio export must produce a canonical runtime asset that KDNA Core can validate and plan before any product loads judgment content.",
  ...
  project.cards.push(
    makeLockedCard(
      ...
      {
        question:
          "Did the runtime export pass KDNA Core validate and LoadPlan?",
      },
      "sc_runtime_001",
    ),
  );
...
test("runtime export emits only canonical runtime entries", () => {
  ...
  assert.equal(
    cbor.decode(exported.files["payload.kdnab"]).profile,
    "kdna.payload.judgment",
  );
  assert.equal(
    cbor.decode(exported.files["payload.kdnab"]).profile_version,
    "0.1.0",
  );
...
test("runtime export keeps authored evolution and excludes Studio lock audit projections", () => {
  ...
  assert.equal(
    payload.evolution.stages.some(
      (stage) => stage.id === "stage_ax_runtime_001",
    ),
    false,
...
test("runtime export preserves declared core relations and extended reasoning semantics", () => {
  ...
  const chain = payload.reasoning.reasoning_chains.find(
    (entry) => entry.id === "reasoning_runtime_001",
  );
  assert.deepEqual(chain.tradeoffs, ["Additional startup work"]);
  assert.deepEqual(chain.conflict_resolution, {
    rule: "Prefer declared protocol constraints.",
  });
...
test("runtime export normalizes every accepted timestamp source to an ISO date-time", () => {
  ...
  assert.equal(
    exported.manifest.authoring.conformance.checked_at,
...
test("runtime export does not turn whitespace-only provenance into an identity", () => {
  ...
  assert.doesNotMatch(
    ...
    /Unknown|author-id-without-name|creator-id-without-name/,
  );
});
...
test("runtime export normalizes string routing fields into arrays", () => {
  ...
  project.cards[0].fields.does_not_apply_when =
    "Editing an internal Studio project";
  ...
  assert.deepEqual(axiom.does_not_apply_when, [
    "Editing an internal Studio project",
  ]);
});
...
test("password export uses a CBOR envelope and loads only with the password", () => {
  ...
  assert.equal(
    exported.manifest.encryption.profile,
    kdnaCore.PASSWORD_PROTECTED_PROFILE,
  );
  assert.equal(
  ...
  try {
    ...
    const capsule = kdnaCore.load(assetPath, {
      password,
      profile: "compact",
      as: "json",
    });
