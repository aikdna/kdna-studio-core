...
function makeLockedCard(type, fields, id) {
  ...
  if (type === "axiom") {
    if (!f.full_statement || f.full_statement.length < 20)
      f.full_statement =
        "A complete testable explanation of this judgment principle with sufficient detail for the agent.";
    if (!f.why || f.why.length < 20)
      f.why =
        "Without this axiom the agent would make incorrect judgment calls producing wrong outputs.";
  }
...
VERDICT: trajectory_changed`;
  ...
  test("createJudgmentDelta builds full report", () => {
    const delta = createJudgmentDelta(
      "@aikdna/writing",
  ...
  test("compareDeltas shows improvement", () => {
    ...
    const betterText = DIFF_TEXT.replace(
      "SAME",
      "Improved boundary awareness with explicit scope limits",
    );
...
describe("Versioning", () => {
  test("diffProjects detects added cards", () => {
    ...
    newProject.cards = [
      makeLockedCard("axiom", { one_sentence: "New axiom." }, "ax_001"),
    ];
  ...
  test("diffProjects detects removed cards", () => {
    const oldProject = createProject("test");
    oldProject.cards = [
      makeLockedCard("axiom", { one_sentence: "Old." }, "ax_001"),
    ];
    const newProject = createProject("test");
    const diff = diffProjects(oldProject, newProject);
  ...
  test("diffProjects detects changed fields", () => {
    ...
    oldProject.cards = [
      makeLockedCard(
        "axiom",
        { one_sentence: "Old text.", full_statement: "Old full." },
        "ax_001",
      ),
    ];
    const newProject = createProject("test");
    newProject.cards = [
      makeLockedCard(
        "axiom",
        { one_sentence: "New text.", full_statement: "New full." },
        "ax_001",
      ),
    ];
    const diff = diffProjects(oldProject, newProject);
  ...
  test("recommendVersionBump: MAJOR for removed axiom", () => {
    const diff = {
      ...
      removed: [{ type: "axiom" }],
      changed: [],
      summary: { added_count: 0, removed_count: 1, changed_count: 0 },
    };
  ...
  test("recommendVersionBump: MINOR for changed fields", () => {
    const diff = {
      ...
      changed: [
        {
          type: "axiom",
          changes: { one_sentence: { before: "old", after: "new" } },
        },
      ],
      summary: { added_count: 0, removed_count: 0, changed_count: 1 },
    };
  ...
  test("generateChangelog produces markdown", () => {
    ...
    newProject.cards = [
      makeLockedCard(
        "axiom",
        { one_sentence: "New axiom for agent judgment." },
        "ax_001",
      ),
    ];
    const diff = diffProjects(oldProject, newProject);
    const changelog = generateChangelog(diff, "0.1.0", "0.2.0", {
      domain: "test",
    });
